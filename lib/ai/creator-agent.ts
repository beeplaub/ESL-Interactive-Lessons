...
export async function projects(userId: string) { const {data,error}=await db().from("creator_agent_projects").select("id,name,updated_at").eq("user_id",userId).order("updated_at",{ascending:false}); if(error) throw new Error("Project storage is unavailable."); return data; }
export async function createProject(userId:string,name:string) { const {data,error}=await db().from("creator_agent_projects").insert({user_id:userId,name:name.trim().slice(0,120)}).select("id,name,updated_at").single(); if(error) throw new Error("Could not create project."); return data; }
export async function renameProject(userId:string,id:string,name:string) { const {data,error}=await db().from("creator_agent_projects").update({name:name.trim().slice(0,120),updated_at:new Date().toISOString()}).eq("id",id).eq("user_id",userId).select("id,name,updated_at").single(); if(error) throw new Error("Could not rename project."); return data; }
export async function projectFiles(userId:string,id:string) { const {data,error}=await db().from("creator_agent_project_files").select("id,name,content,version,enabled,updated_at,creator_agent_projects!inner(user_id)").eq("project_id",id).eq("creator_agent_projects.user_id",userId).order("name"); if(error) throw new Error("Could not read project files."); return data; }
export async function saveProjectFile(userId:string,id:string,file:{id?:string;name:string;content:string;enabled:boolean}) { const own=await db().from("creator_agent_projects").select("id").eq("id",id).eq("user_id",userId).single(); if(own.error) throw new Error("PROJECT_NOT_FOUND"); const q=file.id?db().from("creator_agent_project_files").update({name:file.name,content:file.content,enabled:file.enabled,version:1+1,updated_at:new Date().toISOString()}).eq("id",file.id).eq("project_id",id):db().from("creator_agent_project_files").insert({project_id:id,name:file.name,content:file.content,enabled:file.enabled}); const {data,error}=await q.select().single(); if(error) throw new Error("Could not save project file."); return data; }
  ...
  try {
    ...
    if (request.command === "step") {
      ...
      if (generalRequest) {
        ...
        const ask=(task:string)=>callGemini({ templateKey:"creator_local_agent", variables:{request:JSON.stringify({task,userRequest:state.goal,conversation:state.messages.slice(-8)})}, responseSchema:schema, context:{userId,userRole:"ADMIN",provider:"ollama",featureKey:"creator_local_chat",cache:false}, localAgentOnly:true, fallbackModel:"qwen2.5:7b", validateResponse:(value:unknown)=>z.object({reply:z.string().max(12000)}).parse(value) });
        let reply=await ask("Answer naturally. Do not use database tools or claim to have changed BrenUp. If the user asks for writing, output the complete requested artifact itself, not an explanation or promise. For a dialogue, write every speaker turn.");
        if (reply.reply.trim().length < 100 || (/certainly|here'?s|i apologize|sample dialogue/i.test(reply.reply) && !/(doctor|patient)\s*:/i.test(reply.reply))) reply=await ask("Produce the complete answer now. Output only the requested finished text. Never describe what you will provide and never apologize. For a Doctor-Patient dialogue, include multiple lines beginning Doctor: and Patient:.");
