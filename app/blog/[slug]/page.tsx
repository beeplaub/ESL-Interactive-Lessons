...
import { getBlogRedirect, getPublishedBlogPost, getPublishedBlogPosts, getReadingMinutes, type PublicBlogPost } from "@/lib/blog-public";
...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (databasePost) {
    ...
    const terms = new Set([...databasePost.categoryNames, ...databasePost.tagNames].map((term) => term.toLowerCase()));
    const related = allPosts.filter((post) => post.id !== databasePost.id).sort((left, right) => {
      const leftMatches = [...left.categoryNames, ...left.tagNames].filter((term) => terms.has(term.toLowerCase())).length;
      const rightMatches = [...right.categoryNames, ...right.tagNames].filter((term) => terms.has(term.toLowerCase())).length;
      return rightMatches - leftMatches || (right.publishedAt || right.updatedAt).localeCompare(left.publishedAt || left.updatedAt);
    }).slice(0, 3);
  ...
  return <main className="min-h-0 px-1 pb-10 sm:px-2"><article className="mx-auto w-full max-w-6xl"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><Link href="/blog" className="inline-flex items-center gap-2 text-sm font-extrabold text-[var(--br-brand)] hover:text-[var(--br-action)]"><ArrowLeft className="size-4" /> Back to Journal</Link><header className="mt-5 rounded-3xl bg-surface p-6 shadow-[var(--br-shadow)] sm:p-10"><div className="flex flex-wrap items-center gap-2 text-xs font-bold text-[var(--br-text-muted)]">{article.tags.map((tag) => <span key={tag} className="rounded-full bg-[var(--br-brand-soft)] px-2.5 py-1 text-[var(--br-brand)]">{tag}</span>)}<span className="ml-1 flex items-center gap-1"><Clock3 className="size-3.5" /> {article.readingMinutes} min read</span></div><h1 className="mt-5 max-w-5xl text-3xl font-black leading-tight tracking-[-0.02em] sm:text-4xl">{article.title}</h1><p className="mt-4 max-w-4xl text-base leading-7 text-[var(--br-text-muted)]">{article.description}</p><p className="mt-5 text-sm font-bold text-[var(--br-text-muted)]">{article.author ?? "BrenUp Team"}{article.publishedAt ? ` · ${new Intl.DateTimeFormat("en", { month: "long", day: "numeric", year: "numeric" }).format(new Date(article.publishedAt))}` : ""}</p></header><div className="knowledge-prose mt-5 rounded-3xl bg-surface p-6 shadow-[var(--br-shadow)] sm:p-10" dangerouslySetInnerHTML={{ __html: body }} /></article></main>;
}

function DatabaseArticle({ post, related }: { post: Awaited<ReturnType<typeof getPublishedBlogPost>> & {}; related: PublicBlogPost[] }) {
  if (!post) return null;
  ...
  return <main className="min-h-0 px-1 pb-10 sm:px-2"><BlogViewTracker slug={post.slug} /><article className="mx-auto w-full max-w-6xl"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><Link href="/blog" className="inline-flex items-center gap-2 text-sm font-extrabold text-[var(--br-brand)] hover:text-[var(--br-action)]"><ArrowLeft className="size-4" /> Back to Journal</Link><header className="mt-5 overflow-hidden rounded-3xl bg-surface shadow-[var(--br-shadow)]">{post.coverUrl ? <img src={post.coverUrl} alt="" className="aspect-[16/7] w-full object-cover" /> : null}<div className="p-6 sm:p-10"><div className="flex flex-wrap items-center gap-2 text-xs font-bold text-[var(--br-text-muted)]">{[...post.categoryNames, ...post.tagNames].map((tag) => <span key={tag} className="rounded-full bg-[var(--br-brand-soft)] px-2.5 py-1 text-[var(--br-brand)]">{tag}</span>)}<span className="ml-1 flex items-center gap-1"><Clock3 className="size-3.5" /> {getReadingMinutes(post)} min read</span></div><h1 className="mt-5 max-w-5xl text-3xl font-black leading-tight tracking-[-0.02em] sm:text-4xl">{post.title}</h1>{post.excerpt ? <p className="mt-4 max-w-4xl text-base leading-7 text-[var(--br-text-muted)]">{post.excerpt}</p> : null}<p className="mt-5 text-sm font-bold text-[var(--br-text-muted)]">{post.authorName}{post.publishedAt ? ` · ${new Intl.DateTimeFormat("en", { month: "long", day: "numeric", year: "numeric" }).format(new Date(post.publishedAt))}` : ""}</p></div></header><div className="knowledge-prose mt-5 max-w-none rounded-3xl bg-surface p-6 shadow-[var(--br-shadow)] sm:p-10"><DatabaseBlocks blocks={blocks} /></div><RelatedReading posts={related} /></article></main>;
}

function RelatedReading({ posts }: { posts: PublicBlogPost[] }) {
  if (!posts.length) return null;
  return <section className="mt-5 rounded-3xl bg-surface p-6 shadow-[var(--br-shadow)] sm:p-8"><div className="flex flex-wrap items-end justify-between gap-2"><div><p className="text-xs font-bold uppercase tracking-[0.14em] text-[var(--br-brand)]">Keep exploring</p><h2 className="mt-1 text-xl font-black text-ink">Related reading</h2></div><Link href="/blog" className="text-sm font-bold text-[var(--br-brand)] hover:underline">All articles</Link></div><div className="mt-5 grid gap-3 md:grid-cols-3">{posts.map((related) => <Link key={related.id} href={`/blog/${related.slug}`} className="group overflow-hidden rounded-2xl border border-[var(--br-border)] bg-[var(--br-canvas-elevated)] transition hover:-translate-y-0.5 hover:shadow-sm">{related.coverUrl ? <img src={related.coverUrl} alt="" className="aspect-[16/8] w-full object-cover" /> : null}<div className="p-4"><p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--br-brand)]">{related.categoryNames[0] || related.tagNames[0] || "BrenUp Journal"}</p><h3 className="mt-2 line-clamp-2 font-bold leading-5 text-ink">{related.title}</h3><p className="mt-2 text-xs text-[var(--br-text-muted)]">{getReadingMinutes(related)} min read</p></div></Link>)}</div></section>;
}
