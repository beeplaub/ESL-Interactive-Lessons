...
export default async function CourseQuizPage({
  ...
  const [{ data: questions }, { data: legacyAttempts }, { data: assessmentAttempts }] = await Promise.all([
    ...
    admin.from("assessment_attempts").select("id,quiz_id,legacy_quiz_attempt_id,score,maximum_score,completed_at,submitted_at,created_at,status,grading_source").eq("quiz_id", quizId).eq("user_id", user.id).eq("source_type", "QUIZ").order("completed_at", { ascending: true }),
  ]);
