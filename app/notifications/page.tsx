...
export default async function NotificationsPage() {
  ...
  const [{ data: notices }, { data: preferences }, { data: devices }] = await Promise.all([
    admin.from("user_notifications").select("id,title,detail,href,action_label,tone,category,read_at,archived_at,created_at").eq("user_id", user.id).eq("in_app_enabled", true).is("archived_at", null).or("expires_at.is.null,expires_at.gt." + new Date().toISOString()).order("created_at", { ascending: false }).limit(150),
