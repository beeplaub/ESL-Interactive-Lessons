      ...
      <section className="rounded-[20px] border border-[var(--br-surface-strong)] bg-white p-4 shadow-[0_10px_28px_rgba(0,0,0,.05)] sm:p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {(certificates ?? []).map((certificate) => {
            ...
            return (
              <Link key={certificate.id} href={`/certificates/${certificate.certificate_code}`} className="group rounded-[18px] border border-[var(--br-surface-strong)] bg-[#FCFCFE] p-4 transition hover:-translate-y-0.5 hover:border-[#D9D4F9] hover:shadow-[0_10px_24px_rgba(29,20,83,.08)]">
                <div className="flex items-start justify-between gap-3"><span className="grid size-11 place-items-center rounded-[14px] bg-gradient-to-br from-[var(--br-achievement)] to-[#FF7A00] text-white shadow-sm"><Award className="size-5" /></span><span className="rounded-md bg-[#E7FBF3] px-2 py-1 text-[10px] font-extrabold text-[#00A875]"><ShieldCheck className="mr-1 inline size-3" /> Issued</span></div>
          ...
          {!certificates?.length ? <div className="col-span-full grid min-h-52 place-items-center rounded-[18px] border border-dashed border-[#D9DCE8] bg-[#FAFBFD] p-6 text-center"><div><Award className="mx-auto size-7 text-[#9AA1B8]" /><h3 className="mt-3 font-extrabold text-[var(--br-text)]">Your first certificate is waiting</h3><p className="mt-1 max-w-sm text-sm leading-6 text-[var(--br-text-muted)]">Complete all required items in a course and BrenUp will issue it automatically.</p><Link href="/courses" className="mt-4 inline-flex rounded-xl bg-[var(--br-chart-primary)] px-4 py-2.5 text-xs font-extrabold text-white">Browse courses</Link></div></div> : null}
        </div>
      </section>
    </LearnerAppShell>
  );
