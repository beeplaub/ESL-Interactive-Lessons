      ...
      <section className="rounded-[20px] border border-[var(--br-surface-strong)] bg-surface p-4 shadow-[var(--br-shadow)] sm:p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {(certificates ?? []).map((certificate) => {
            ...
            return (
              <Link key={certificate.id} href={`/certificates/${certificate.certificate_code}`} className="group rounded-[18px] border border-[var(--br-surface-strong)] bg-[var(--br-surface)] p-4 transition hover:-translate-y-0.5 hover:border-[var(--br-brand-soft)] hover:shadow-[var(--br-shadow)]">
                <div className="flex items-start justify-between gap-3"><span className="grid size-11 place-items-center rounded-[14px] bg-gradient-to-br from-[var(--br-achievement)] to-[var(--br-action)] text-on-dark shadow-sm"><Award className="size-5" /></span><span className="rounded-md bg-[var(--br-success-soft)] px-2 py-1 text-[10px] font-extrabold text-[var(--br-chart-secondary)]"><ShieldCheck className="mr-1 inline size-3" /> Issued</span></div>
          ...
          {!certificates?.length ? <div className="col-span-full grid min-h-52 place-items-center rounded-[18px] border border-dashed border-[var(--br-border)] bg-[var(--br-surface)] p-6 text-center"><div><Award className="mx-auto size-7 text-[var(--br-text-muted)]" /><h3 className="mt-3 font-extrabold text-[var(--br-text)]">Your first certificate is waiting</h3><p className="mt-1 max-w-sm text-sm leading-6 text-[var(--br-text-muted)]">Complete all required items in a course and BrenUp will issue it automatically.</p><Link href="/courses" className="mt-4 inline-flex rounded-xl bg-[var(--br-chart-primary)] px-4 py-2.5 text-xs font-extrabold text-on-dark">Browse courses</Link></div></div> : null}
        </div>
      </section>
    </LearnerAppShell>
  );
