    ...
    <LearnerAppShell active="certificates">
      ...
      <section className="rounded-[20px] border border-[var(--br-surface-strong)] bg-white p-4 shadow-[0_10px_28px_rgba(0,0,0,.05)] sm:p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {!certificates?.length ? <div className="col-span-full grid min-h-52 place-items-center rounded-[18px] border border-dashed border-[#D9DCE8] bg-[#FAFBFD] p-6 text-center"><div><Award className="mx-auto size-7 text-[#9AA1B8]" /><h3 className="mt-3 font-extrabold text-[#35405F]">Your first certificate is waiting</h3><p className="mt-1 max-w-sm text-sm leading-6 text-[var(--br-text-muted)]">Complete all required items in a course and BrenUp will issue it automatically.</p><Link href="/courses" className="mt-4 inline-flex rounded-xl bg-[var(--br-chart-primary)] px-4 py-2.5 text-xs font-extrabold text-white">Browse courses</Link></div></div> : null}
        </div>
      </section>
    </LearnerAppShell>
  );
