...
export default async function AccountPage() {
  ...
  const [
    ...
    adminSupabase.from("course_enrollments").select("*, courses(id,title,level,topic,status,deleted_at,thumbnail_path,cover_image_path,duration_minutes,estimated_completion_minutes)").eq("user_id", user.id).order("enrolled_at", { ascending: false }),
  ...
  const instructorMap = await getCourseInstructorMap(enrolledCourseIds);
  const { data: enrolledCourseItems } = enrolledCourseIds.length
    ? await adminSupabase.from("course_items").select("course_id").in("course_id", enrolledCourseIds)
    : { data: [] as { course_id: string }[] };
  const enrolledLessonCountByCourse = new Map<string, number>();
  for (const item of enrolledCourseItems ?? []) {
    enrolledLessonCountByCourse.set(item.course_id, (enrolledLessonCountByCourse.get(item.course_id) ?? 0) + 1);
  }
  ...
  const [{ data: assignments }, { data: liveClasses }, { data: practiceTasks }] = await Promise.all([
    classIds.length ? adminSupabase.from("class_assignments").select("id,title,item_type,course_id,lesson_id,quiz_id,due_at,required_score").in("class_id", classIds).order("due_at", { ascending: true, nullsFirst: false }).limit(4) : Promise.resolve({ data: [] }),
      ...
      <div className="flex min-w-0 gap-5">
        <section className="flex min-w-0 flex-1 flex-col gap-5">
          {isAdminLearnerView ? (
            <form action={switchToAdminView} className="rounded-[20px] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button className="rounded-xl bg-amber-900 px-3 py-2 text-xs font-bold text-on-dark">Switch to Admin</button>
              </div>
            </form>
          ) : null}
          ...
          <div className="grid gap-5 md:grid-cols-2">
            <DashboardCard className="p-5 md:p-6"><SectionHeader title="This Week: Assignments" href="/assignments" small />{assignments?.length ? <div className="space-y-2">{assignments.map((assignment) => <Link key={assignment.id} href={assignmentHref(assignment)} className="flex items-center gap-3 rounded-xl p-3 transition hover:bg-[var(--br-surface-muted)]"><span className="size-2 rounded-full bg-[var(--br-action)] ring-4 ring-[var(--br-action)]/10"/><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{assignment.title || assignment.item_type}</p><p className="text-xs text-[var(--br-text-muted)]">{assignment.due_at ? `Due ${new Date(assignment.due_at).toLocaleDateString()}` : "No due date"}</p></div><ChevronRight className="size-4 text-[var(--br-text-muted)]" /></Link>)}</div> : <EmptyMini text="No assignments this week." href="/assignments" label="View assignments" />}</DashboardCard>
            <DashboardCard className="p-5 md:p-6"><SectionHeader title="This Week: Live Classes" href="/live-classes" small />{liveClasses?.length ? <div className="space-y-2">{liveClasses.map((session) => <Link key={session.id} href={`/live/${session.id}`} className="flex items-center gap-3 rounded-xl border border-[var(--br-border)] p-3 transition hover:border-[var(--br-action)]"><span className="grid size-11 shrink-0 place-items-center rounded-lg bg-[var(--br-brand)] text-xs font-bold text-on-dark">{session.scheduled_at ? new Date(session.scheduled_at).getDate() : "LIVE"}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{session.title}</p><p className="text-xs text-[var(--br-text-muted)]">{session.status === "LIVE" ? "Live now" : session.scheduled_at ? new Date(session.scheduled_at).toLocaleString() : "Time to be confirmed"}</p></div><span className="rounded-lg bg-[var(--br-action)] px-3 py-1.5 text-xs font-bold text-on-dark">{session.status === "LIVE" ? "Join" : "View"}</span></Link>)}</div> : <EmptyMini text="No live classes scheduled." href="/live-classes" label="View classes" />}</DashboardCard>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3"><DashboardCard className="p-5"><SectionHeader title="Language Profile" href="/language-profile" small /><p className="text-sm text-[var(--br-text-muted)]">Current level <b className="text-[var(--br-dark-card)]">{currentLevel}</b></p><div className="mt-4 h-3 overflow-hidden rounded-full bg-[var(--br-surface-strong)]"><div className="h-full rounded-full bg-[var(--br-action)]" style={{width:`${Math.max(12, activeLevelIndex * 16 + 20)}%`}}/></div><Link href="/language-profile" className="mt-4 block rounded-lg bg-[var(--br-surface-muted)] py-2 text-center text-xs font-bold text-[var(--br-brand)]">Full skill map</Link></DashboardCard><DashboardCard className="p-5"><SectionHeader title="Last Quiz" href="/quizzes" small />{quizAttempts?.[0] ? <><p className="text-3xl font-bold text-[var(--br-success)]">{quizAttempts[0].total ? Math.round(quizAttempts[0].score/quizAttempts[0].total*100) : 0}%</p><p className="mt-2 text-sm font-bold">{quizAttempts[0].quizzes?.title || "Quiz"}</p><p className="text-xs text-[var(--br-text-muted)]">{new Date(quizAttempts[0].completed_at).toLocaleDateString()}</p></> : <EmptyMini text="Your recent quiz will appear here." href="/quizzes" label="Take a quiz" />}</DashboardCard><DashboardCard className="p-5"><SectionHeader title="My Rank" href="/leaderboard" small /><p className="text-3xl font-bold text-[var(--br-achievement)]">Keep climbing</p><p className="mt-2 text-sm text-[var(--br-text-muted)]">Earn points through quizzes and completed learning.</p><Link href="/leaderboard" className="mt-4 inline-flex text-xs font-bold text-[var(--br-brand)]">View leaderboard <ChevronRight className="size-3"/></Link></DashboardCard></div>
          <section><SectionHeader title="Enrolled Courses" href="/courses" /><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{activeCourseEnrollments.map((enrollment, index) => { const course = Array.isArray(enrollment.courses) ? enrollment.courses[0] : enrollment.courses; return course ? <MarketingCourseCard key={enrollment.id} course={course} lessonCount={enrolledLessonCountByCourse.get(enrollment.course_id) ?? 0} tone={index} instructors={instructorMap.get(enrollment.course_id)} /> : null; })}{!activeCourseEnrollments.length ? <EmptyMini text="Your enrolled courses will appear here." href="/courses" label="Browse courses" /> : null}<Link href="/courses" className="grid min-h-48 place-items-center rounded-[20px] border-2 border-dashed border-[var(--br-border)] p-6 text-center text-sm font-bold text-[var(--br-text-muted)] transition hover:border-[var(--br-action)] hover:text-[var(--br-action)]">Explore new courses</Link></div></section>
          <section><SectionHeader title="My Certificates" href="/certificates" /><div className="flex gap-4 overflow-x-auto pb-1">{certificates?.length ? certificates.map((certificate) => <Link key={certificate.id} href={`/certificates/${certificate.certificate_code}`} className="w-64 shrink-0 rounded-[20px] border border-[var(--br-border)] bg-surface p-5 shadow-sm"><Award className="size-8 text-[var(--br-achievement)]"/><p className="mt-4 font-bold">{certificate.courses?.title || "Course certificate"}</p><p className="mt-1 text-xs text-[var(--br-text-muted)]">Earned {new Date(certificate.issued_at).toLocaleDateString()}</p></Link>) : <EmptyMini text="Complete an enrolled course to earn a certificate." href="/courses" label="Browse courses" />}</div></section>
        </section>
    ...
    <div className="flex flex-col gap-4 rounded-[24px] bg-gradient-to-br from-[var(--br-brand-strong)] via-[var(--br-dark-card)] to-[var(--br-dark-card)] p-5 text-on-dark shadow-[var(--br-shadow)] md:p-7 min-[1100px]:flex-row min-[1100px]:gap-8">
      ...
      {levelTestSummary ? (
        ...
        <div className="min-[1100px]:w-[220px] rounded-[18px] bg-white/[.07] p-5">
          ...
          <Link href="/level-test" className="mt-4 flex items-center justify-center gap-1.5 rounded-xl bg-gradient-to-br from-[var(--br-chart-primary)] to-[var(--br-brand)] px-4 py-3 text-xs font-semibold text-on-dark">Take Level Test <ChevronRight className="size-[13px]" /></Link>
        </div>
      )}
    </div>
  );
...
function PracticeTile({ href, icon: Icon, label, sub, tone, disabled }: { href: string; icon: React.ElementType; label: string; sub: string; tone: "pink" | "blue" | "orange" | "green" | "purple" | "gray"; disabled?: boolean }) {
  ...
  const content = (
    <>
      <div className={`grid size-9 place-items-center rounded-[14px] bg-gradient-to-br ${tones[tone]} min-[540px]:size-11`}><Icon className="size-5 text-on-dark" /></div>
