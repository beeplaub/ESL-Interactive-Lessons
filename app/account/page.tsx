    ...
    >
      ...
      <div className="flex min-w-0 gap-5">
        <section className="flex min-w-0 flex-1 flex-col gap-5">
          ...
          <section><SectionHeader title="Enrolled Courses" href="/courses" /><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">{activeCourseEnrollments.map((enrollment, index) => { const course = Array.isArray(enrollment.courses) ? enrollment.courses[0] : enrollment.courses; return course ? <MarketingCourseCard key={enrollment.id} course={course} lessonCount={enrolledLessonCountByCourse.get(enrollment.course_id) ?? 0} tone={index} /> : null; })}{!activeCourseEnrollments.length ? <EmptyMini text="Your enrolled courses will appear here." href="/courses" label="Browse courses" /> : null}<Link href="/courses" className="grid min-h-48 place-items-center rounded-[20px] border-2 border-dashed border-[var(--br-border)] p-6 text-center text-sm font-bold text-[var(--br-text-muted)] transition hover:border-[var(--br-action)] hover:text-[var(--br-action)]">Explore new courses</Link></div></section>
