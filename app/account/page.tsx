...
function CourseHomeCard({href,title,level,progress,tone}:{href:string;title:string;level:string;progress:number;tone:number}) { const colors=["from-[var(--br-brand)] to-[var(--br-dark-card)]","from-[var(--br-action)] to-[var(--br-danger)]","from-[var(--br-success)] to-[var(--br-success)]"]; return <Link href={href} className="overflow-hidden rounded-[20px] border border-[var(--br-border)] bg-surface shadow-sm transition hover:-translate-y-0.5"><div className={`h-24 bg-gradient-to-br ${colors[tone%colors.length]} p-4`}><span className="rounded bg-white/20 px-2 py-1 text-[10px] font-bold text-on-dark">{level}</span></div><div className="p-4"><p className="truncate font-bold">{title}</p><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[var(--br-surface-strong)]"><div className="h-full bg-[var(--br-action)]" style={{width:`${progress}%`}}/></div><p className="mt-2 text-xs text-[var(--br-text-muted)]">{Math.round(progress)}% complete</p></div></Link> }
