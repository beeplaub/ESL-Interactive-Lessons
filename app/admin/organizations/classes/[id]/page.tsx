...
import { ArrowLeft, BookOpen, CalendarDays, CheckCircle2, ClipboardList, GraduationCap, Trash2, UsersRound } from "lucide-react";
import { requireAdmin } from "@/lib/auth";
import { createAdminClient } from "@/lib/supabase/admin";
import { removeClassAssignment } from "@/app/admin/organizations/actions";
...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [courseRows, lessonRows, quizRows, courseProgressRows, lessonProgressRows, quizAttemptRows, levelResults] = await Promise.all([
  ...
  return (
    <main className="min-w-0 overflow-hidden">
      <Link href="/admin/organizations" className="inline-flex items-center gap-1.5 text-sm font-semibold text-black/55 hover:text-moss"><ArrowLeft size={15} /> Organizations</Link>
      <div className="mt-4 flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-moss">Class report</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">{klass.name}</h1><p className="mt-2 text-sm text-black/60">{organization?.name ?? "Independent class"} · {klass.level ?? "All levels"} · Teacher: {teacherName}</p></div><div className="rounded-xl border border-black/10 bg-white px-4 py-3 text-right shadow-sm"><p className="text-2xl font-semibold">{members?.length ?? 0}</p><p className="text-xs font-semibold uppercase tracking-wide text-black/45">Learners</p></div></div>
      {klass.description ? <p className="mt-3 max-w-3xl text-sm leading-6 text-black/60">{klass.description}</p> : null}

      <section className="mt-6 rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><UsersRound size={18} className="text-moss" /><h2 className="font-semibold">Learners</h2></div><div className="mt-4 divide-y divide-black/10">{(members ?? []).map((member) => { const profile = profiles.get(member.user_id); const name = profile?.full_name?.trim() || [profile?.first_name, profile?.last_name].filter(Boolean).join(" ") || "Learner"; return <div key={member.id} className="flex flex-wrap items-center justify-between gap-2 py-3"><div><Link href={`/admin/students/${member.user_id}`} className="font-semibold text-moss hover:underline">{name}</Link><p className="mt-0.5 text-xs text-black/50">{emails.get(member.user_id) || "No email"}{profile?.cefr_level ? ` · ${profile.cefr_level}` : ""}</p></div><span className="text-xs font-medium text-black/45">Joined {new Date(member.joined_at).toLocaleDateString()}</span></div>; })}{!(members?.length) ? <p className="py-6 text-center text-sm text-black/55">No learners in this class yet.</p> : null}</div></section>

      <section className="mt-5 rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><ClipboardList size={18} className="text-moss" /><h2 className="font-semibold">Assigned work</h2></div><div className="mt-4 space-y-3">{typedAssignments.map((assignment) => { const resource = assignment.item_type === "COURSE" ? courses.get(assignment.course_id ?? "") : assignment.item_type === "LESSON" ? lessons.get(assignment.lesson_id ?? "") : assignment.item_type === "QUIZ" ? quizzes.get(assignment.quiz_id ?? "") : null; const title = assignment.title?.trim() || resource?.title || (assignment.item_type === "LEVEL_TEST" ? "English level test" : "Assigned item"); let completedCount = 0; for (const member of members ?? []) { if (assignment.item_type === "COURSE" && assignment.course_id && (courseProgress.get(`${member.user_id}:${assignment.course_id}`) ?? 0) >= 100) completedCount++; if (assignment.item_type === "LESSON" && assignment.lesson_id && lessonProgress.get(`${member.user_id}:${assignment.lesson_id}`)) completedCount++; if (assignment.item_type === "QUIZ" && assignment.quiz_id) { const attempt = latestQuiz.get(`${member.user_id}:${assignment.quiz_id}`); const percent = attempt?.total ? Math.round((attempt.score / attempt.total) * 100) : null; if (percent !== null && percent >= (assignment.required_score ?? 0)) completedCount++; } if (assignment.item_type === "LEVEL_TEST" && latestLevel.has(member.user_id)) completedCount++; } const percent = members?.length ? Math.round((completedCount / members.length) * 100) : 0; return <article key={assignment.id} className="rounded-xl border border-black/10 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{title}</h3><span className="rounded-full bg-skywash px-2 py-0.5 text-[10px] font-bold text-ink">{assignment.item_type.replace("_", " ")}</span></div><p className="mt-1 text-xs text-black/50">{resource?.level ?? ""}{assignment.due_at ? `${resource?.level ? " · " : ""}Due ${new Date(assignment.due_at).toLocaleDateString()}` : ""}{assignment.required_score ? ` · Target ${assignment.required_score}%` : ""}</p></div><form action={removeClassAssignment.bind(null, assignment.id)}><button className="inline-flex items-center gap-1 rounded-md border border-red-200 px-2.5 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50"><Trash2 size={13} /> Remove</button></form></div><div className="mt-4 flex items-center gap-3"><span className="text-sm font-semibold text-moss">{completedCount}/{members?.length ?? 0} complete</span><span className="h-2 flex-1 overflow-hidden rounded-full bg-black/5"><span className="block h-full rounded-full bg-moss" style={{ width: `${percent}%` }} /></span><span className="text-xs font-semibold text-black/50">{percent}%</span></div></article>; })}{!typedAssignments.length ? <div className="py-8 text-center"><BookOpen className="mx-auto text-black/30" size={24} /><p className="mt-2 text-sm text-black/55">No work has been assigned to this class yet.</p></div> : null}</div></section>
    </main>
  );
}
