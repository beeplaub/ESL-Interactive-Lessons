  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-3">
        <select name="classId" required disabled={!classes.length} className="rounded-md border border-black/15 px-3 py-2 text-sm disabled:bg-slate-50">
          <option value="">Choose class...</option>
          {classes.map((klass) => <option key={klass.id} value={klass.id}>{klass.name}</option>)}
        </select>
        <input name="email" type="email" required placeholder="Learner email address" className="rounded-md border border-black/15 px-3 py-2 text-sm" />
        {message ? <p className={`text-xs font-semibold ${isError ? "text-red-600" : "text-emerald-700"}`}>{message}</p> : null}
        <button disabled={isPending || !classes.length} className="inline-flex w-fit items-center gap-2 rounded-md bg-moss px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"><UserPlus size={15} /> {isPending ? "Adding…" : "Add learner"}</button>
      </div>
    </form>
  );
}
