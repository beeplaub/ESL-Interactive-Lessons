...
export default async function CreatorWorkspacePage() {
  ...
  const [{ data: projects }, { data: tasks }, { data: notes }, { data: resources }] = await Promise.all([
    ...
    admin.from("creator_notes").select("id,title,body,updated_at").eq("creator_id", user.id).order("updated_at", { ascending: false }).limit(100),
    admin.from("creator_resources").select("id,title,value,resource_type,description").eq("creator_id", user.id).order("updated_at", { ascending: false }).limit(100),
  ]);
