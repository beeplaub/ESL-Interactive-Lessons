...
export default async function AdminPlansPage() {
  ...
  return (
    <main className="min-w-0 space-y-6">
      <section className="rounded-20 bg-gradient-to-br from-[#24105e] via-[#4020a9] to-[#7437f2] p-5 text-white shadow-lg sm:p-7">
      ...
      <section className="space-y-4">
        ...
        <div className="overflow-x-auto br-card rounded-20">
          <table className="min-w-[900px] w-full text-left text-sm">
            <thead className="border-b border-black/10 bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="p-3">Creator</th><th className="p-3">Plan</th><th className="p-3">Status</th><th className="p-3">Override</th></tr></thead>
            <tbody>
              {creators.map((creator) => {
                ...
                return <tr key={creator.id} className="border-b border-black/5 last:border-0"><td className="p-3 font-semibold text-ink">{name}<p className="mt-0.5 text-xs font-normal text-slate-500">{creator.role}</p></td><td className="p-3"><form action={assignCreatorPlan} className="flex items-center gap-2"><input type="hidden" name="userId" value={creator.id} /><select name="planId" defaultValue={plan?.id ?? plans.find((row) => row.plan_key === "FREE")?.id} className="rounded-lg border border-black/15 bg-white px-2 py-1.5 text-xs">{plans.map((option) => <option key={option.id} value={option.id}>{option.name}</option>)}</select><select name="status" defaultValue={subscription?.status ?? "ACTIVE"} className="rounded-lg border border-black/15 bg-white px-2 py-1.5 text-xs"><option value="ACTIVE">Active</option><option value="TRIALING">Trialing</option><option value="PAST_DUE">Past due</option><option value="CANCELLED">Cancelled</option><option value="EXPIRED">Expired</option></select><button className="rounded-lg bg-dark px-2 py-1.5 text-xs font-bold text-white">Save</button></form></td><td className="p-3 text-xs text-slate-600">{subscription?.billing_interval ?? "Manual"}</td><td className="p-3"><CreatorOverrideForm creatorId={creator.id} overrides={overrides.filter((row) => row.user_id === creator.id)} /></td></tr>;
              })}
...
function CreatorOverrideForm({ creatorId, overrides }: { creatorId: string; overrides: Override[] }) {
  ...
  return <form action={saveCreatorEntitlementOverride} className="flex items-center gap-2"><input type="hidden" name="userId" value={creatorId} /><select name="featureKey" defaultValue={first?.feature_key ?? "COURSES"} className="rounded-lg border border-black/15 bg-white px-2 py-1.5 text-xs">{entitlementKeys.map((key) => <option key={key} value={key}>{featureLabels[key]}</option>)}</select><select name="isEnabled" defaultValue={first?.is_enabled === null || !first ? "INHERIT" : first.is_enabled ? "ENABLED" : "DISABLED"} className="rounded-lg border border-black/15 bg-white px-2 py-1.5 text-xs"><option value="INHERIT">Inherit</option><option value="ENABLED">Enable</option><option value="DISABLED">Disable</option></select><input name="limitValue" type="number" min="0" placeholder="Limit" defaultValue={first?.limit_value ?? ""} className="w-16 rounded-lg border border-black/15 px-2 py-1.5 text-xs" /><button className="rounded-lg border border-black/15 px-2 py-1.5 text-xs font-bold text-ink">Set</button></form>;
}
