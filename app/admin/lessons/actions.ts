...
function blockContentFromForm(blockType: string, formData: FormData): Json {
  ...
  if (blockType === "REVIEW_CHECKLIST") {
    return {
      title: nullableText(formData.get("title")),
      intro: nullableText(formData.get("intro")),
      items: splitLines(formData.get("items")),
      require_completion: formData.get("require_completion") === "on"
    };
  }
  if (blockType === "QUOTE") {
    return {
  ...
  if (blockType === "SPACE") {
    const height = Math.min(100, Math.max(0, Number(formData.get("height")) || 0));
    return { height };
  }
  return {};
}
