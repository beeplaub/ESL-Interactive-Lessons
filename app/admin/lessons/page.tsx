  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid gap-3 md:hidden">
        {(lessons ?? []).map((lesson) => (
          <article key={lesson.id} className="rounded-lg border border-black/10 bg-white p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h2 className="truncate font-semibold">{lesson.title}</h2>
                <p className="mt-1 text-sm text-black/55">{lesson.level} · {lesson.topic}</p>
                <p className="mt-1 text-xs text-black/40">{new Date(lesson.created_at).toLocaleDateString()}</p>
              </div>
              <span className="rounded-full bg-moss/10 px-2 py-1 text-xs font-medium text-moss">{lesson.status}</span>
            </div>
            <div className="mt-4 grid grid-cols-[1fr_auto_auto] gap-2">
              <Link className="inline-flex items-center justify-center gap-2 rounded-md border border-black/15 px-3 py-2 text-sm font-medium hover:bg-black/5" href={`/admin/lessons/${lesson.id}/builder`}>
              ...
              <form action={deleteLesson.bind(null, lesson.id)}>
                <button className="h-full rounded-md border border-coral/30 px-3 py-2 text-coral hover:bg-coral/10" aria-label="Delete">
                  <Trash2 size={16} />
                </button>
              </form>
            </div>
        ...
        {!lessons?.length ? (
          <div className="rounded-lg border border-black/10 bg-white p-8 text-center text-black/60 shadow-sm">No lessons yet.</div>
        ) : null}
      </div>
