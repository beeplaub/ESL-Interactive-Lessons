  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/admin/lessons" className="inline-flex items-center gap-1 text-sm font-semibold text-[var(--br-text-muted)] hover:text-[var(--br-text-muted)]">
        ...
        <div className="divide-y divide-black/10">
          {(lessons ?? []).map((lesson) => (
            <div key={lesson.id} className="grid gap-3 p-4 md:grid-cols-[1.4fr_0.9fr_0.7fr_1.2fr] md:items-center">
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-xs text-[var(--br-text-muted)]">{lesson.level} · {lesson.topic ?? "No topic"}</p>
              </div>
          ...
          {(lessons?.length ?? 0) === 0 ? (
            <div className="p-8 text-center text-sm text-[var(--br-text-muted)]">
              <BookOpen className="mx-auto mb-3 text-[var(--br-text-muted)]" size={32} />
