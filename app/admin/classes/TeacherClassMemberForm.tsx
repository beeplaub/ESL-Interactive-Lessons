...
export function TeacherClassMemberForm({ classId }: { classId: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-sm text-[var(--br-text-muted)]">Use the email address attached to their BrenUp account.</p>
      <input name="classId" type="hidden" value={classId} />
      <div className="mt-4 flex flex-col gap-3 sm:flex-row">
        <input name="email" type="email" required placeholder="learner@email.com" className="min-w-0 flex-1 rounded-md border border-[var(--br-border)] px-3 py-2 text-sm" />
        <button disabled={isPending} className="inline-flex items-center justify-center gap-2 rounded-md bg-moss px-4 py-2 text-sm font-semibold text-on-dark disabled:opacity-60">
