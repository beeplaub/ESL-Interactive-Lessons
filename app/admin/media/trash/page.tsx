  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          <Link href="/admin/media" className="inline-flex items-center gap-1 text-sm font-semibold text-black/60 hover:text-black">
      ...
      <section className="overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm">
        <div className="hidden grid-cols-[1.6fr_0.7fr_0.9fr_1.2fr] gap-3 border-b border-black/10 bg-slate-50 p-3 text-xs font-semibold uppercase tracking-wide text-black/50 md:grid">
        ...
        <div className="divide-y divide-black/10">
          {(assets ?? []).map((asset) => (
            <div key={asset.id} className="grid gap-3 p-4 md:grid-cols-[1.6fr_0.7fr_0.9fr_1.2fr] md:items-center">
              <div className="min-w-0">
                ...
                <p className="mt-1 truncate text-xs text-black/50">{asset.url}</p>
              </div>
              <span className="w-fit rounded-full bg-black/10 px-2.5 py-1 text-xs font-semibold text-black/50">{asset.type}</span>
          ...
          {(assets?.length ?? 0) === 0 ? (
            <div className="p-8 text-center text-sm text-black/55">
              <Images className="mx-auto mb-3 text-black/25" size={32} />
