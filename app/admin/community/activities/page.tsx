...
export default async function CommunityActivitiesPage() {
  ...
  const { data: activities } = await admin.from("community_practice_activities").select("id,title,description,prompt,follow_up_prompt,status,cefr_level,skill,circle_id").eq("created_by", user.id).order("created_at", { ascending: false });
