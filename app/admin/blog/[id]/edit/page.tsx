...
export default async function AdminBlogPostEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!post) notFound();
  const [{ data: mappings }, { data: tagMappings }, { data: categories }, { data: tags }, { data: media }, { data: revisionRows }] = await Promise.all([
  ...
  return <BlogPostEditor post={editable} role={session.blogRole} categories={categories ?? []} tags={tags ?? []} media={media ?? []} revisions={revisions} />;
}
