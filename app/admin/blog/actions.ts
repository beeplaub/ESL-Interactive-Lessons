...
export async function createBlogPost(
  ...
  try {
    ...
    const { data, error } = await admin
      ...
      .select("id,title,slug,content_version")
      .single();
    if (error || !data)
      return {
        success: false,
...
export async function changeBlogPostStatus(
  ...
  try {
    ...
    if (!session.blogRole)
      return {
        success: false,
        error: "You do not have access to BrenUp Journal.",
      };
    const admin = createAdminClient();
    const { data: post, error: readError } = await admin
      .from("blog_posts")
      .select(
        "id,title,slug,excerpt,content,content_text,content_version,created_by",
      )
      .eq("id", postId)
      .maybeSingle();
    if (readError || !post)
      return { success: false, error: readError?.message || "Post not found." };
    ...
    if (status === "PUBLISHED" && !canPublish)
      return { success: false, error: "Only an editor can publish a post." };
    if (["APPROVED", "CHANGES_REQUESTED"].includes(status) && !canReview)
      return {
        success: false,
        error: "Only a reviewer can change the review decision.",
      };
    if (!canPublish && !canReview && !ownsPost)
      return { success: false, error: "You can only update your own posts." };
    if (
      role === "CONTRIBUTOR" &&
      !["DRAFT", "IN_REVIEW", "TRASH"].includes(status)
    )
      return {
        success: false,
        error: "Contributors can save drafts or submit work for review.",
      };
    ...
    if (status === "DRAFT" && post.created_by === session.user.id) {
      patch.deleted_at = null;
      patch.archived_at = null;
    }
    ...
    await admin
      .from("blog_posts")
      .update({ content_version: post.content_version + 1 })
      .eq("id", postId);
    refreshBlog(postId);
    return { success: true };
  } catch (error) {
...
export async function duplicateBlogPost(
  ...
  try {
    ...
    const { data: original, error: readError } = await admin
      .from("blog_posts")
      .select(
        "title,excerpt,content,content_text,visibility,cover_asset_id,og_asset_id,primary_category_id,seo_title,seo_description,social_title,social_description,canonical_url,primary_keyword,allow_index,is_featured,is_commenting_enabled,toc_enabled,toc_title,toc_include_h3,toc_include_h4,toc_include_h5,toc_include_h6",
      )
      .eq("id", postId)
      .maybeSingle();
    ...
    if (error || !data)
      return {
        success: false,
        error: error?.message || "Could not duplicate the post.",
      };
    await writeRevision({
      postId: data.id,
      version: 1,
...
export async function saveBlogPost(
  ...
  try {
    ...
    if (!validContent(input.content))
      return {
        success: false,
        error: "The article content has an unsupported block.",
      };
    const admin = createAdminClient();
    const { data: post, error: readError } = await admin
      .from("blog_posts")
      .select("id,created_by,slug,content_version,status")
      .eq("id", postId)
      .maybeSingle();
    if (readError || !post)
      return { success: false, error: readError?.message || "Post not found." };
    const canEditAny = ["PLATFORM_ADMIN", "EDITOR"].includes(session.blogRole);
    if (!canEditAny && post.created_by !== session.user.id)
      return { success: false, error: "You can only edit your own posts." };
    if (
      session.blogRole === "CONTRIBUTOR" &&
      !["DRAFT", "CHANGES_REQUESTED", "IN_REVIEW"].includes(post.status)
    )
      return {
        success: false,
        error: "Contributors cannot edit this post in its current state.",
      };
    ...
    const slug = requestedSlug
      ? await uniqueSlug(requestedSlug, postId)
      : cleanTitle === "Untitled post"
        ? post.slug
        : await uniqueSlug(cleanTitle, postId);
    ...
    const nextVersion =
      event === "AUTOSAVED" ? post.content_version : post.content_version + 1;
    const { error } = await admin
      .from("blog_posts")
      .update({
        title: cleanTitle,
        ...
        primary_category_id: categoryIds.includes(input.primaryCategoryId || "")
        ...
        toc_enabled: input.tocEnabled !== false,
        toc_title: input.tocTitle?.trim() || "On this page",
        toc_include_h3: input.tocIncludeH3 !== false,
        toc_include_h4: input.tocIncludeH4 !== false,
        toc_include_h5: Boolean(input.tocIncludeH5),
        toc_include_h6: Boolean(input.tocIncludeH6),
    ...
    if (event !== "AUTOSAVED")
      await writeRevision({
        ...
        metadata: {
          ...
          allowIndex: input.allowIndex !== false,
...
export async function scheduleBlogPost(
  ...
  try {
    ...
    if (
      ...
      return { success: false, error: "Choose a future publication time." };
    const admin = createAdminClient();
    const { data: post, error: readError } = await admin
      .from("blog_posts")
      .select("title,slug,excerpt,content,content_text,content_version")
      .eq("id", postId)
      .maybeSingle();
    if (readError || !post)
      return { success: false, error: readError?.message || "Post not found." };
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Could not schedule the post.",
    };
  }
}
...
export async function publishDueBlogPosts() {
  ...
  for (const post of posts ?? []) {
    ...
    await writeRevision({
      ...
      metadata: { scheduledPublish: true },
    });
    refreshBlog(post.id);
    published += 1;
  }
...
export async function saveBlogTag(
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Could not save the tag.",
    };
  }
}

export async function setBlogEditorMember(
  formData: FormData,
): Promise<{ success: boolean; error?: string }> {
  try {
    const session = await getBlogSession();
    if (session.blogRole !== "PLATFORM_ADMIN")
      return {
        success: false,
        error: "Only a platform administrator can manage Journal roles.",
      };
    const userId = String(formData.get("userId") || "");
    const role = String(formData.get("role") || "");
    if (
      !userId ||
      !["EDITOR", "AUTHOR", "CONTRIBUTOR", "REVIEWER"].includes(role)
    )
      return { success: false, error: "Choose a valid team member and role." };
    const admin = createAdminClient();
    ...
    if (
      !profile ||
      !["ADMIN", "TEACHER", "SCHOOL_ADMIN"].includes(String(profile.role))
    )
      return {
        success: false,
        error: "Journal roles can only be assigned to BrenUp staff.",
      };
    const { error } = await admin
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Could not update the Journal role.",
    };
  }
}

export async function setBlogTaxonomyStatus(
  kind: "category" | "tag",
  id: string,
  isActive: boolean,
): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    const admin = createAdminClient();
    const { error } = await admin
      .from(kind === "category" ? "blog_categories" : "blog_tags")
...
export async function restoreBlogRevision(
  ...
  try {
    ...
    const [
      ...
      admin
        .from("blog_posts")
        .select("id,created_by,slug,content_version")
        .eq("id", postId)
        .maybeSingle(),
      admin
        .from("blog_post_revisions")
        .select("id,post_id,title,slug,excerpt,content,content_text,metadata")
        .eq("id", revisionId)
        .eq("post_id", postId)
        .maybeSingle(),
    ]);
    if (postError || !post || revisionError || !revision)
      return {
        success: false,
        error:
          postError?.message || revisionError?.message || "Revision not found.",
      };
    const canEditAny = ["PLATFORM_ADMIN", "EDITOR"].includes(session.blogRole);
    if (!canEditAny && post.created_by !== session.user.id)
      return { success: false, error: "You can only restore your own post." };
    const metadata = (
      ...
      .update({
        ...
        primary_category_id:
          typeof metadata.primaryCategoryId === "string"
            ...
            : null,
        seo_title:
          typeof metadata.seoTitle === "string" ? metadata.seoTitle : null,
        seo_description:
          typeof metadata.seoDescription === "string"
            ? metadata.seoDescription
            : null,
        social_title:
          typeof metadata.socialTitle === "string"
            ? metadata.socialTitle
            : null,
        social_description:
          typeof metadata.socialDescription === "string"
...
export async function saveBlogPostPattern(input: {
  name: string;
  description?: string;
  content: { type: "doc"; content: BlogBlock[] };
  ...
  try {
    ...
    if (
      !session.blogRole ||
      !["PLATFORM_ADMIN", "EDITOR", "AUTHOR"].includes(session.blogRole)
    )
      return {
        success: false,
        error: "Your Journal role cannot save reusable patterns.",
      };
    const name = input.name.trim();
    if (!name || !validContent(input.content) || !input.content.content.length)
      return { success: false, error: "Give this non-empty pattern a name." };
    if (
      input.scope === "GLOBAL" &&
      !["PLATFORM_ADMIN", "EDITOR"].includes(session.blogRole)
    )
      return {
        success: false,
        error: "Only editors can share a pattern with the whole Journal team.",
      };
    const admin = createAdminClient();
    const { error } = await admin
      .from("blog_post_patterns")
      .insert({
        name,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Could not save the reusable pattern.",
    };
  }
}

export async function archiveBlogPostPattern(
  patternId: string,
): Promise<{ success: boolean; error?: string }> {
  try {
    const session = await getBlogSession();
    ...
    const admin = createAdminClient();
    const { data: pattern, error: readError } = await admin
      .from("blog_post_patterns")
      .select("created_by,scope")
      .eq("id", patternId)
      .maybeSingle();
    if (readError || !pattern)
      return {
        success: false,
        error: readError?.message || "Pattern not found.",
      };
    if (
      pattern.created_by !== session.user.id &&
      !["PLATFORM_ADMIN", "EDITOR"].includes(session.blogRole)
    )
      return {
        success: false,
        error: "You can only archive your own pattern.",
      };
    const { error } = await admin
      .from("blog_post_patterns")
      .update({
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Could not archive the pattern.",
    };
  }
}

export async function addBlogEditorialComment(
  postId: string,
  body: string,
  ...
  try {
    ...
    if (!cleanBody)
      return { success: false, error: "Write a review note first." };
    const admin = createAdminClient();
    ...
    if (
      !["PLATFORM_ADMIN", "EDITOR", "REVIEWER"].includes(session.blogRole) &&
      post.created_by !== session.user.id
    )
      return {
        success: false,
        error: "You can only comment on your own draft.",
      };
    const { error } = await admin
      .from("blog_editorial_comments")
      .insert({
        post_id: postId,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Could not save the review note.",
    };
  }
}

export async function resolveBlogEditorialComment(
  commentId: string,
): Promise<{ success: boolean; error?: string }> {
  try {
    const session = await getBlogSession();
    ...
    if (readError || !comment)
      return {
        success: false,
        error: readError?.message || "Review note not found.",
      };
    const { error } = await admin
      .from("blog_editorial_comments")
      .update({
      ...
      .eq("id", commentId);
    if (error) return { success: false, error: error.message };
    revalidatePath(`/admin/blog/${comment.post_id}/edit`);
    return { success: true };
  } catch (error) {
    return {
      success: false,
      error:
        error instanceof Error
          ? error.message
          : "Could not resolve the review note.",
    };
  }
}
...
export async function importLegacyJournalPosts(): Promise<{
  success: boolean;
  imported?: number;
  error?: string;
}> {
  try {
    ...
    if (session.blogRole !== "PLATFORM_ADMIN")
      return {
        success: false,
        error: "Only a platform administrator can import legacy Journal posts.",
      };
    const admin = createAdminClient();
    let imported = 0;
    for (const entry of getKnowledgeEntries("blog")) {
      ...
      const content = {
        type: "doc" as const,
        content: legacyMarkdownBlocks(entry.content),
      };
      ...
      if (error || !post)
        return {
          success: false,
          error: error?.message || "Could not import a legacy article.",
        };
      await writeRevision({
        postId: post.id,
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Could not import the legacy Journal posts.",
    };
  }
}
