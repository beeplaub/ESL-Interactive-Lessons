  ...
  if (!organization) {
    return (
      <main>
        ...
        <p className="mt-2 text-sm text-[var(--br-text-muted)]">Your account is not linked to a school yet.</p>
      </main>
    );
  }
  ...
  return (
    <main className="min-w-0">
      <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div className="flex min-w-0 items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-[var(--br-text-muted)]">Manage your people, classes, assignments, and school identity.</p>
          </div>
        </div>
          ...
          {organizations?.map((item) => (
            <Link
              ...
              className={`rounded-md px-3 py-2 text-xs font-semibold ${
                item.id === organization.id ? "bg-dark text-on-dark" : "border border-[var(--br-border)] hover:bg-black/5"
              }`}
            >
      ...
      <section className="mt-5 grid gap-3 sm:grid-cols-2">
        <Link href={`/admin/school/learners?org=${organization.id}`} className="group rounded-xl border border-[var(--br-border)] bg-surface p-4 shadow-sm transition hover:border-violetglow/35 hover:shadow-md">
          ...
          <p className="mt-2 text-sm text-[var(--br-text-muted)]">Find every learner attached to this school’s classes.</p>
        </Link>
        <Link href={`/admin/school/reports?org=${organization.id}`} className="group rounded-xl border border-[var(--br-border)] bg-surface p-4 shadow-sm transition hover:border-violetglow/35 hover:shadow-md">
          ...
          <p className="mt-2 text-sm text-[var(--br-text-muted)]">Review assignment completion and class activity in one place.</p>
        </Link>
        <Link href={`/admin/school/guardians?org=${organization.id}`} className="group rounded-xl border border-[var(--br-border)] bg-surface p-4 shadow-sm transition hover:border-violetglow/35 hover:shadow-md">
          ...
          <p className="mt-2 text-sm text-[var(--br-text-muted)]">Invite a parent or guardian to view one learner’s progress.</p>
        </Link>
      </section>
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_minmax(300px,.8fr)]">
        <div className="rounded-xl border border-[var(--br-border)] bg-surface p-5 shadow-sm">
          ...
          <div className="mt-4 divide-y divide-black/10">
            {(classes ?? []).map((klass) => (
              <Link key={klass.id} href={`/admin/classes/${klass.id}`} className="flex items-center justify-between gap-3 py-4 hover:bg-surface-muted">
                <div className="min-w-0"><p className="truncate font-semibold">{klass.name}</p><p className="mt-1 text-xs text-[var(--br-text-muted)]">{klass.level || "All levels"} · {klass.teacher_id ? names.get(klass.teacher_id) || "Teacher" : "No teacher assigned"}</p></div>
                <span className="rounded-full bg-moss/10 px-2.5 py-1 text-[11px] font-bold text-moss">{klass.status}</span>
              </Link>
            ))}
            {!classes?.length ? <p className="py-8 text-center text-sm text-[var(--br-text-muted)]">No classes yet. Add your first class above.</p> : null}
          </div>
        </div>
        ...
        <aside className="space-y-5">
          <section className="rounded-xl border border-[var(--br-border)] bg-surface p-5 shadow-sm">
            ...
            <div className="mt-4 divide-y divide-black/10">
              {(assignments ?? []).slice(0, 8).map((assignment) => (
                <div key={assignment.id} className="py-3"><p className="font-semibold">{assignment.title || assignment.item_type.replace("_", " ")}</p><p className="mt-1 text-xs text-[var(--br-text-muted)]">{classMap.get(assignment.class_id) || "Class"}{assignment.due_at ? ` · Due ${new Date(assignment.due_at).toLocaleDateString()}` : ""}</p></div>
              ))}
              {!assignments?.length ? <p className="py-5 text-center text-sm text-[var(--br-text-muted)]">No assignments yet.</p> : null}
            </div>
          </section>
          ...
          <form action={updateSchoolBranding.bind(null, organization.id)} className="rounded-xl border border-[var(--br-border)] bg-surface p-5 shadow-sm">
            <h2 className="font-semibold">School identity</h2>
            <div className="mt-3 grid gap-3"><input name="brandName" defaultValue={organization.brand_name ?? ""} placeholder="Display name" className="rounded-md border border-[var(--br-border)] px-3 py-2 text-sm" /><input name="logoUrl" type="url" defaultValue={organization.logo_url ?? ""} placeholder="Logo image URL" className="rounded-md border border-[var(--br-border)] px-3 py-2 text-sm" /><label className="text-xs font-semibold text-[var(--br-text-muted)]">Accent color <input name="accentColor" type="color" defaultValue={organization.accent_color ?? "var(--br-chart-primary)"} className="ml-2 h-8 w-12 rounded border border-[var(--br-border)] bg-surface p-0.5 align-middle" /></label><button className="w-fit rounded-md border border-[var(--br-border)] px-3 py-2 text-sm font-semibold hover:bg-black/5">Save identity</button></div>
          </form>
        </aside>
      </section>
    </main>
