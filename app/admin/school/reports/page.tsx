...
export default async function SchoolReportsPage({
  ...
  if (!organization) return <main><h1 className="text-2xl font-semibold">School reports</h1><p className="mt-2 text-sm text-black/60">No school is linked to this account.</p></main>;
  ...
  const [courseRows, lessonRows, quizRows, courseProgressRows, lessonProgressRows, quizAttemptRows, levelResultRows, profileRows] = await Promise.all([
    ...
    learnerIds.length ? admin.from("profiles").select("id,full_name,first_name,last_name").in("id", learnerIds) : Promise.resolve({ data: [] }),
  ]);
  ...
  const learnerNames = new Map((profileRows.data ?? []).map((row) => [row.id, row.full_name?.trim() || [row.first_name, row.last_name].filter(Boolean).join(" ") || "Learner"]));
  ...
  return (
    <main className="min-w-0">
      <Link href={`/admin/school?org=${organization.id}`} className="inline-flex items-center gap-1.5 text-sm font-semibold text-black/55 hover:text-violetglow"><ArrowLeft size={15} /> School workspace</Link>
      <div className="mt-4 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-violetglow">{organization.brand_name || organization.name}</p><h1 className="mt-1 text-2xl font-semibold sm:text-3xl">School reports</h1><p className="mt-2 max-w-2xl text-sm text-black/60">Completion is based on assigned work: finished courses and lessons, qualifying quiz attempts, and completed level tests.</p></div><Link href={`/admin/school/learners?org=${organization.id}`} className="rounded-md border border-black/15 px-3 py-2 text-sm font-semibold hover:bg-black/5">View learners</Link></div>

      <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="School learners" value={learnerIds.length} /><Metric label="Active classes" value={(classes ?? []).filter((row) => row.status === "ACTIVE").length} /><Metric label="Assigned items" value={assignmentRows.length} /><Metric label="Completion" value={`${completionRate}%`} /></section>

      <section className="mt-6 grid gap-5 xl:grid-cols-[minmax(0,1.08fr)_minmax(320px,.92fr)]">
        <div className="rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><ClipboardList size={18} className="text-violetglow" /><h2 className="font-semibold">Assignment completion</h2></div><div className="mt-4 space-y-3">{assignmentRows.map((assignment) => <article key={assignment.id} className="rounded-lg border border-black/10 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{assignment.title}</h3><span className="rounded-full bg-skywash px-2 py-0.5 text-[10px] font-bold text-ink">{assignment.item_type.replace("_", " ")}</span>{assignment.level ? <span className="text-xs font-semibold text-black/45">{assignment.level}</span> : null}</div><p className="mt-1 text-xs text-black/50">{assignment.due_at ? `Due ${new Date(assignment.due_at).toLocaleDateString()} · ` : ""}{assignment.required_score ? `${assignment.required_score}% quiz target` : "No minimum score"}</p></div><span className="text-sm font-bold text-violetglow">{assignment.completed}/{assignment.learners}</span></div><Progress value={assignment.percent} /></article>)}{!assignmentRows.length ? <Empty icon={<ClipboardList size={25} />} text="No assignments exist yet. Add assignments from the school workspace." /> : null}</div></div>
        <aside className="space-y-5"><section className="rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><GraduationCap size={18} className="text-violetglow" /><h2 className="font-semibold">Class progress</h2></div><div className="mt-4 space-y-3">{classRows.map((klass) => <Link key={klass.id} href={`/admin/classes/${klass.id}`} className="block rounded-lg border border-black/10 p-3 hover:border-violetglow/35"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate font-semibold">{klass.name}</p><p className="mt-1 text-xs text-black/50">{klass.learners} learners · {klass.assignments} assignments</p></div><span className="text-sm font-bold text-violetglow">{klass.completion}%</span></div><Progress value={klass.completion} /></Link>)}{!classRows.length ? <Empty icon={<GraduationCap size={25} />} text="No classes to report on yet." /> : null}</div></section><section className="rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2"><UsersRound size={18} className="text-violetglow" /><h2 className="font-semibold">Learner activity</h2></div><div className="mt-4 divide-y divide-black/10">{learnerActivity.slice(0, 8).map((learner) => <div key={learner.id} className="flex items-center justify-between gap-3 py-3"><p className="min-w-0 truncate text-sm font-semibold">{learner.name}</p><p className="shrink-0 text-xs font-bold text-black/55">{learner.complete}/{learner.total} complete</p></div>)}{!learnerActivity.length ? <Empty icon={<UsersRound size={25} />} text="Learners will appear once they join a class." /> : null}</div></section></aside>
      </section>

      <section className="mt-5 rounded-xl border border-violetglow/15 bg-violetglow/[0.04] p-4 text-sm text-black/60"><div className="flex gap-3"><CheckCircle2 className="mt-0.5 shrink-0 text-violetglow" size={18} /><p><strong className="text-black/80">What this measures:</strong> assigned-work completion. Course outcome attainment and detailed language evidence remain available in each course’s outcomes and reports once question-to-outcome mappings have been configured.</p></div></section>
    </main>
  );
}
...
function Empty({ icon, text }: { icon: React.ReactNode; text: string }) {
  return <div className="grid min-h-28 place-items-center p-4 text-center text-sm text-black/50"><div>{icon}<p className="mt-2">{text}</p></div></div>;
}
