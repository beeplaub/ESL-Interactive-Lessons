...
async function sessionAccess(id: string) {
  ...
  const admin = createAdminClient(); const [{ data: session }, profile] = await Promise.all([
    admin.from("live_sessions").select("id,class_id,course_id,teacher_id,lesson_id,status,navigation_locked,timer_ends_at,current_slide_number").eq("id", id).maybeSingle(),
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "lock") {
    ...
    const activity = await liveClassActivity(admin, session, activityId);
