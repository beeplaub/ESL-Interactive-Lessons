...
async function teacherSession(id: string) {
  ...
  if (!session || (session.teacher_id !== user.id && !isPlatformAdmin(profile?.role))) return { user, session: null };
  return { user, session };
}

export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const { session } = await teacherSession(id);
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const { user, session } = await teacherSession(id);
  if (!user || !session) return NextResponse.json({ error: "Teacher access required" }, { status: 403 });
