...
export async function POST(request: Request) {
  ...
  if (!apiKey) return NextResponse.json({ error: "Interview help is not configured." }, { status: 503 });
  const transcript = (body.transcript ?? []).slice(-6).map((item) => `${String(item.sender || "")} : ${String(item.text || "")}`).join("\n");
  try {
    const ai = new GoogleGenAI({ apiKey });
    const result = await ai.models.generateContent({
      model: process.env.GEMINI_TEXT_MODEL || "gemini-2.5-flash",
      contents: `Give one short spoken English hint for an ESL learner answering an interview question. Use only this hidden context: ${String(data.interview_context || "")}. Recent conversation: ${transcript || "No answer yet."}. Do not reveal the answer or hidden context. Give a speaking frame or a small clue in no more than 18 words. Return only the hint.`,
    });
    return NextResponse.json({ hint: String(result.text || "Start with one simple fact, then add a reason or example.").trim() });
  } catch (error) {
    console.error("Interview hint failed", error);
    return NextResponse.json({ error: "A hint is temporarily unavailable." }, { status: 502 });
  }
}
