...
export default async function GuardianPage() {
  ...
  if (!link) return <main className="mx-auto grid min-h-screen max-w-2xl place-items-center px-4"><section className="rounded-3xl border border-black/10 bg-white p-7 text-center shadow-sm"><LockKeyhole className="mx-auto text-violetglow" size={30} /><h1 className="mt-3 text-2xl font-extrabold text-ink">No learner is linked yet</h1><p className="mt-2 text-sm leading-6 text-slate-600">Ask the learner’s school to send a guardian invitation to this email address.</p></section></main>;
  ...
  return <main className="min-h-screen bg-[#F6F7FB] px-4 py-8 text-[#14172B] sm:px-6"><div className="mx-auto max-w-5xl"><header className="rounded-[26px] bg-gradient-to-br from-[#1A1060] via-[#24105e] to-[#6C3BFF] p-6 text-white shadow-lg sm:p-8"><p className="text-xs font-bold uppercase tracking-[.16em] text-white/65">BrenUp guardian view</p><h1 className="mt-2 text-3xl font-extrabold">{name}’s learning progress</h1><p className="mt-2 text-sm text-white/75">Read-only progress for your linked learner. This view cannot change lessons, results, or assignments.</p></header><section className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Classes" value={(classes ?? []).length} /><Metric label="Assigned work" value={assignmentsRows.length} /><Metric label="Completed" value={completedAssignments.length} /><Metric label="Recent quiz average" value={averageScore === null ? "—" : `${averageScore}%`} /></section><section className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><div className="space-y-5"><Panel icon={<ClipboardList size={18} />} title="Next steps"><div className="divide-y divide-black/10">{nextAssignments.map((assignment) => <div key={assignment.id} className="py-3"><p className="font-semibold">{assignment.title || titles.get(`${assignment.item_type}:${assignment.course_id ?? assignment.lesson_id ?? assignment.quiz_id ?? ""}`) || "Assigned learning item"}</p><p className="mt-1 text-xs text-black/50">{assignment.item_type.replace("_", " ")}{assignment.due_at ? ` · Due ${new Date(assignment.due_at).toLocaleDateString()}` : ""}</p></div>)}{!nextAssignments.length ? <Empty text="No outstanding assigned work right now." /> : null}</div></Panel><Panel icon={<TrendingUp size={18} />} title="Recent learning activity"><div className="divide-y divide-black/10">{(quizAttempts ?? []).slice(0, 5).map((attempt, index) => <div key={`${attempt.quiz_id}-${index}`} className="flex items-center justify-between gap-3 py-3"><p className="text-sm font-semibold">Quiz practice</p><p className="text-xs font-bold text-violetglow">{attempt.total ? `${Math.round((Number(attempt.score) / Number(attempt.total)) * 100)}%` : "Attempted"}</p></div>)}{!(quizAttempts ?? []).length ? <Empty text="Recent quiz and lesson activity will appear here." /> : null}</div></Panel></div><aside className="space-y-5"><Panel icon={<GraduationCap size={18} />} title="Strengths and focus"><p className="text-sm leading-6 text-black/60">{averageScore !== null && averageScore >= 70 ? `${name} is showing secure recent quiz performance.` : `${name} is building confidence through regular practice.`}</p><p className="mt-3 text-sm leading-6 text-black/60">A useful next step is completing the listed assignments and revisiting any quiz below its target score.</p></Panel><Panel icon={<BookOpen size={18} />} title="Course progress"><div className="space-y-3">{(courseProgress ?? []).slice(0, 5).map((progress) => <div key={progress.course_id}><div className="flex justify-between gap-3 text-sm"><span className="font-semibold">Course</span><span className="font-bold text-violetglow">{progress.progress_percent}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/5"><span className="block h-full rounded-full bg-violetglow" style={{ width: `${progress.progress_percent}%` }} /></div></div>)}{!(courseProgress ?? []).length ? <Empty text="Course progress will appear after enrolled learning begins." /> : null}</div></Panel><section className="rounded-xl border border-violetglow/15 bg-violetglow/[.05] p-4 text-xs leading-5 text-black/60"><CheckCircle2 className="mb-2 text-violetglow" size={17} />Privacy matters: this account can only view {name}’s linked learning data.</section></aside></section></div></main>;
}
function Metric({ label, value }: { label: string; value: string | number }) { return <div className="rounded-xl border border-black/10 bg-white p-4 shadow-sm"><p className="text-2xl font-extrabold">{value}</p><p className="mt-1 text-xs font-bold uppercase tracking-wide text-black/45">{label}</p></div>; }
function Panel({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) { return <section className="rounded-xl border border-black/10 bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-violetglow">{icon}<h2 className="font-semibold text-ink">{title}</h2></div><div className="mt-4">{children}</div></section>; }
function Empty({ text }: { text: string }) { return <p className="py-5 text-center text-sm text-black/50">{text}</p>; }
