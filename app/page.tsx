        ...
        <div className="relative z-10 mx-auto grid w-full max-w-[1440px] gap-8 px-6 py-12 sm:px-12 lg:grid-cols-[45%_55%] lg:items-center lg:px-12 lg:py-16">
          ...
          <div className="relative flex h-[500px] w-full items-center justify-center sm:h-[600px]">
            ...
            <div className="hero-float-reverse absolute bottom-[10%] right-[4%] z-30 flex items-center gap-3 rounded-xl border border-white/40 bg-white/85 p-4 shadow-md backdrop-blur sm:right-[10%]">
              <span className="rounded-lg bg-[#f2b705]/10 p-2 text-xl">🔥</span><div><div className="text-xs font-semibold text-[var(--br-text-muted)]">Study Streak</div><div className="text-sm font-bold text-[#1b1b3a]">12 days in a row</div></div>
            </div>
      ...
      <section className="bg-[#f1f1f6] px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-[1200px]">
          <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div><h2 className="text-4xl font-bold tracking-tight text-[#1b1b3a]">Featured Courses</h2><p className="mt-2 text-[var(--br-text-muted)]">Curated paths from our top linguistic experts.</p></div>
...
function ValueCard({ icon: Icon, title, text, tone }: { icon: React.ElementType; title: string; text: string; tone: "coral" | "indigo" | "green" }) {
  const tones = { coral: "bg-[var(--br-action)]/10 text-[var(--br-action)] group-hover:bg-[var(--br-action)]", indigo: "bg-[#28235a]/10 text-[#28235a] group-hover:bg-[#28235a]", green: "bg-[#2fae7a]/10 text-[#2fae7a] group-hover:bg-[#2fae7a]" };
  return (
    <article className="group rounded-[20px] border border-[var(--br-border)] bg-white p-8 transition duration-300 hover:-translate-y-1 hover:shadow-xl"><span className={`grid size-16 place-items-center rounded-xl transition-colors duration-300 group-hover:text-white ${tones[tone]}`}><Icon className="size-8" /></span><h3 className="mt-6 text-2xl font-bold text-[#1b1b3a]">{title}</h3><p className="mt-3 leading-6 text-[var(--br-text-muted)]">{text}</p></article>
  );
}
...
function CourseCard({ course, lessonCount, tone }: { course: { id: string; title: string; level: string | null; thumbnail_path: string | null; cover_image_path: string | null; duration_minutes: number | null; estimated_completion_minutes: number | null }; lessonCount: number; tone: number }) {
  ...
  const tones = ["from-[#ffb199] to-[#3e3a72]", "from-[#3e3a72] to-[#aba6e6]", "from-[#f2b705] to-[#28235a]", "from-[#2fae7a] to-[#28235a]", "from-[var(--br-action)] to-[#f2b705]", "from-[#28235a] to-[#ffb199]"];
  ...
  return (
    <Link href={`/courses/${course.id}`} className="group overflow-hidden rounded-[20px] border border-[var(--br-border)] bg-white transition hover:shadow-2xl">
      ...
      <div className="p-6"><h3 className="line-clamp-2 text-xl font-semibold text-[#1b1b3a]">{course.title}</h3><div className="mt-4 flex gap-4 text-sm text-[var(--br-text-muted)]">{duration ? <span className="inline-flex items-center gap-1"><Clock3 className="size-4" /> {duration} min</span> : null}<span className="inline-flex items-center gap-1"><PlayCircle className="size-4" /> {lessonCount} {lessonCount === 1 ? "Lesson" : "Lessons"}</span></div></div>
    </Link>
  );
}
