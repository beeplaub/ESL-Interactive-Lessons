...
import { Hand, MessageCircle, Send, UsersRound, Vote } from "lucide-react";
  ...
  return <aside className="space-y-3">
    <section className="rounded-xl border border-black/10 bg-white p-3 shadow-sm">
      ...
      <div className="mt-3 max-h-52 space-y-2 overflow-y-auto pr-1">
        {groupMessages?.length ? groupMessages.map((item) => <div key={item.id} className="rounded-lg bg-slate-50 p-2"><p className="text-[10px] font-extrabold text-violetglow">{item.sender_name}{item.channel === "TEACHER" ? " · private" : item.channel === "GROUP" ? " · group" : ""}</p><p className="mt-0.5 text-xs text-black/70">{item.body}</p></div>) : <p className="py-4 text-center text-xs text-black/45">{channel === "GROUP" ? "Your group chat is ready." : "Class chat will appear here."}</p>}
      </div>
    ...
    {teacher ? <section className="rounded-xl border border-black/10 bg-white p-3 shadow-sm"><div className="flex items-center gap-1.5"><UsersRound size={16} className="text-violetglow" /><h2 className="text-sm font-extrabold">Group work</h2></div><p className="mt-1 text-xs text-black/50">Create balanced temporary groups for this live class.</p><div className="mt-3 grid grid-cols-3 gap-1.5">{[2, 3, 4].map((count) => <button key={count} type="button" onClick={() => announce({ action: "createGroups", count })} className="rounded-md border border-violetglow/20 bg-[#F8F6FF] px-2 py-2 text-xs font-bold text-violetglow">{count} groups</button>)}</div>{data?.groups.length ? <div className="mt-3 space-y-1">{data.groups.map((group) => <p key={group.id} className="rounded-md bg-slate-50 px-2 py-1.5 text-xs"><b>{group.name}</b> · {group.memberIds.length} learners</p>)}</div> : null}</section> : null}
