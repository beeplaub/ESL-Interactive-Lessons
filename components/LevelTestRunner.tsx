      ...
      <div className="mx-auto max-w-5xl">
        <header className="sticky top-0 z-30 rounded-[18px] border border-[var(--br-surface-strong)] bg-white/95 p-3 shadow-[var(--br-shadow)] backdrop-blur sm:p-4">
          <div className="flex items-center gap-3">
            ...
            ) : <span className="rounded-full bg-[color-mix(in_srgb,var(--br-success)_12%,var(--br-surface))] px-3 py-1.5 text-xs font-bold text-[var(--br-success)]">Untimed</span>}
          </div>
          <div className="mt-3 flex gap-1 overflow-hidden">
            {questions.map((question, index) => {
              ...
              return <span key={question.id} className={`h-1.5 min-w-[3px] flex-1 rounded-full transition-colors ${index === activeIndex ? "bg-[var(--br-chart-primary)]" : answered ? "bg-[var(--br-success)]" : "bg-[var(--br-border)]"}`} />;
            })}
          </div>
        </header>
        ...
        <div className={`mt-4 grid gap-4 ${passage ? "lg:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]" : ""}`}>
          {passage ? (
            <aside className="rounded-[20px] border border-[var(--br-surface-strong)] bg-surface p-5 shadow-[var(--br-shadow)] lg:sticky lg:top-28 lg:max-h-[calc(100vh-130px)] lg:overflow-y-auto">
              ...
              <p className="mt-4 whitespace-pre-line text-sm font-medium leading-7 text-[var(--br-text-muted)]">{passage.body}</p>
            </aside>
          ) : null}
          ...
          <section className="rounded-[20px] border border-[var(--br-surface-strong)] bg-surface p-5 shadow-[var(--br-shadow)] sm:p-7">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-[var(--br-info-soft)] px-3 py-1 text-[11px] font-extrabold text-[var(--br-info)]">{current.cefrBand}</span>
            ...
            <div className="mt-6">
              {current.questionType === "FILL" ? (
                <input
                  ...
                  className="w-full rounded-[14px] border-2 border-[var(--br-border)] px-4 py-3 text-base font-semibold outline-none focus:border-[var(--br-chart-primary)]"
                />
              ) : (
                  ...
                  {current.options.map((option) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-3 rounded-[14px] border-2 p-3 text-left transition-all sm:p-4 ${selected ? "border-[var(--br-chart-primary)] bg-[var(--br-surface-muted)]" : "border-[var(--br-surface-strong)] bg-surface hover:border-[var(--br-border)]"}`}
                      >
                        <span className={`grid size-9 shrink-0 place-items-center rounded-[11px] text-sm font-black ${selected ? "bg-[var(--br-chart-primary)] text-on-dark" : "bg-[var(--br-surface-muted)] text-[var(--br-text-muted)]"}`}>{selected ? <Check className="size-4" /> : option.key}</span>
