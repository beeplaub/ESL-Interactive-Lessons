...
export function LiveMeetingPanel({ sessionId, meetingUrl, callingEnabled }: { sessionId: string; meetingUrl?: string | null; callingEnabled: boolean }) {
  ...
  return <section className="rounded-xl border border-[var(--br-border)] bg-surface p-3" aria-label="Class calling">
    <div className="flex flex-wrap items-center gap-3"><div className="w-full min-w-0 md:w-auto md:flex-1"><h2 className="text-sm font-bold">Classroom call</h2><p className="mt-1 text-xs text-[var(--br-text-muted)]">{callingEnabled ? "Join with your microphone and camera off, then choose when to speak." : meeting ? `Open ${meeting.label} for audio, then return here for your lesson and activities.` : "In-app calling is unavailable. Check the classroom calling configuration or add a meeting link."}</p></div>
      {callingEnabled ? <button type="button" disabled={joining} onClick={() => joined ? leave() : void join()} className="min-h-11 rounded-lg bg-[var(--br-brand)] px-3 text-sm font-bold text-on-dark disabled:opacity-50">{joining ? "Joining…" : joined ? "Leave call" : "Join classroom call"}</button> : <span role="status" className="text-xs text-[var(--br-text-muted)]">In-app call unavailable · an administrator needs to enable calling.</span>}
