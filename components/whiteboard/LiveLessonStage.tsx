...
export function LiveLessonStage({ sessionId, initialLesson, initialLessonId, live, role, initialSlideNumber }: { sessionId: string; initialLesson: ReactNode; initialLessonId?: string | null; live: boolean; role: "TEACHER" | "STUDENT"; initialSlideNumber?: number }) {
  ...
  const [slideNumber, setSlideNumber] = useState(initialSlideNumber ?? 1);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    const onSlide = async (event: Event) => {
      const item = (event as CustomEvent<{ sessionId: string; item?: { item_type: string; lesson_id?: string | null; slide_id?: string | null; slide_number?: number | null } }>).detail;
      if (item?.sessionId !== sessionId || item.item?.item_type !== "LESSON_SLIDE" || !item.item.lesson_id || item.item.lesson_id === initialLessonId) { if (item?.item?.item_type === "LESSON_SLIDE" && item.item.lesson_id === initialLessonId) { setSlideNumber(item.item.slide_number ?? 1); setData(null); } return; }
      setSlideNumber(item.item.slide_number ?? 1);
      setLoading(true);
