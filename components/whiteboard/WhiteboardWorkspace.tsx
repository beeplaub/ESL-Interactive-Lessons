...
export function WhiteboardWorkspace({ sessionId, status, lesson, overview, meetingUrl, callingEnabled, slides = [], title = "Live classroom", level, startedAt }: { sessionId: string; status: string; lesson: ReactNode; overview?: ReactNode; meetingUrl?: string | null; callingEnabled: boolean; slides?: ClassroomSlide[]; title?: string; level?: string | null; startedAt?: string | null }) {
  ...
  return <div ref={frame} onKeyDown={keyboard} className={`wb-workspace ${expanded ? "wb-expanded" : ""}`}>
    ...
    <div className="wb-layout">
      <nav className="wb-slides" aria-label="Live class slides"><LivePlaylistPanel sessionId={sessionId} teacher={teacher} live={status === "LIVE"} /><div className="wb-encourage"><svg viewBox="0 0 240 125" aria-hidden="true"><path d="M0 94 74 30 126 85 177 13 240 90V125H0Z" fill="#e9ddff" /><path d="M0 112 70 55 135 112 196 61 240 96V125H0Z" fill="#cce7f7" /><path d="M180 70V5l17 8-17 7" stroke="#9654f7" fill="#9654f7" /></svg><span>Small steps.<br />Big conversations. <b>♥</b></span></div>
      </nav>
      <main className="wb-main">
        ...
        <div hidden={view !== "board"}>
          ...
          <div hidden={view !== "lesson"} className="wb-existing-lesson"><LiveLessonStage sessionId={sessionId} initialLesson={lesson} initialLessonId={null} live={status === "LIVE"} role={teacher ? "TEACHER" : "STUDENT"} initialSlideNumber={settings.slide} /></div>
      </main>
