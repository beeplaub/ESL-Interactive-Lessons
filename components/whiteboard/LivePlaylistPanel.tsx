...
export function LivePlaylistPanel({ sessionId, teacher, live }: { sessionId: string; teacher: boolean; live: boolean }) {
  ...
  return <section className="wb-playlist" aria-label="Live class slides"><div className="wb-playlist-heading"><b>Live slides</b>{teacher && live ? <button type="button" onClick={() => setPickerOpen(true)}><Plus size={15} /> Add slide</button> : null}</div>{items.length ? <ol className="wb-playlist-items">{items.map((item, index) => <li key={item.id}><GripVertical size={14} /><span><b>{index + 1}. {item.title}</b><small>{item.item_type === "WHITEBOARD" ? "Collaborative whiteboard" : "Course lesson slide"}</small></span>{teacher && live ? <><button type="button" aria-label={`Move ${item.title} up`} disabled={!index} onClick={() => void move(index, -1)}><ChevronUp size={14} /></button><button type="button" aria-label={`Move ${item.title} down`} disabled={index === items.length - 1} onClick={() => void move(index, 1)}><ChevronDown size={14} /></button><button type="button" aria-label={`Remove ${item.title}`} onClick={() => void remove(item.id)}><Trash2 size={14} /></button></> : null}</li>)}</ol> : <p className="wb-help">Add a whiteboard or course lesson slide to begin.</p>}{message ? <p role="alert" className="wb-help">{message}</p> : null}{pickerOpen ? <div className="wb-playlist-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPickerOpen(false); }}><div className="wb-playlist-modal" role="dialog" aria-modal="true" aria-labelledby="wb-playlist-title"><div className="wb-playlist-modal-head"><div><small>LIVE CLASSROOM</small><h3 id="wb-playlist-title">Add a slide</h3><p>Choose a blank whiteboard or any published slide from the assigned course.</p></div><button type="button" aria-label="Close add slide" onClick={() => setPickerOpen(false)}>×</button></div><button className="wb-playlist-choice" type="button" onClick={() => void add({ type: "WHITEBOARD", title: `Whiteboard ${items.filter((item) => item.item_type === "WHITEBOARD").length + 1}` }).then(() => setPickerOpen(false))}><Plus size={19} /><span><b>Blank whiteboard</b><small>Start a collaborative teaching slide</small></span></button><div className="wb-playlist-divider"><span>COURSE SLIDES</span></div><div className="wb-playlist-source-list">{sources.length ? sources.map((source) => <button className="wb-playlist-choice" type="button" key={source.slideId} onClick={() => void add({ type: "LESSON_SLIDE", lessonId: source.lessonId, slideId: source.slideId }).then(() => setPickerOpen(false))}><BookOpenIcon /><span><b>{source.slideTitle}</b><small>{source.lessonTitle}</small></span></button>) : <p className="wb-help">No published course slides are available for this class.</p>}</div></div></div> : null}</section>;
}
