  ...
  return (
    <div className="grid gap-5">
      <label className="block text-sm font-medium">
        Paste quiz text
        <textarea value={text} onChange={(event) => setText(event.target.value)} rows={16} className="mt-2 w-full rounded-md border border-[var(--br-border)] px-3 py-3 font-mono text-sm leading-6" />
      </label>
      <div className="flex flex-wrap gap-3">
        <button type="button" onClick={parse} className="rounded-md bg-dark px-4 py-2 text-sm font-medium text-on-dark">Parse</button>
        {parsed ? (
          <>
            <button type="button" disabled={isPending} onClick={() => submit("DRAFT")} className="rounded-md border border-[var(--br-border)] px-4 py-2 text-sm font-medium disabled:opacity-50">Save as Draft</button>
            <button type="button" disabled={isPending} onClick={() => submit("PUBLISHED")} className="rounded-md bg-moss px-4 py-2 text-sm font-medium text-on-dark disabled:opacity-50">Publish</button>
          </>
        ) : null}
      </div>
      ...
      {parsed ? (
        <section className="rounded-lg border border-[var(--br-border)] bg-surface p-4 shadow-sm">
          <div className="grid gap-3 sm:grid-cols-3">
            <input value={parsed.title} onChange={(event) => setParsed({ ...parsed, title: event.target.value })} className="rounded-md border border-[var(--br-border)] px-3 py-2" />
            <input value={parsed.topic} onChange={(event) => setParsed({ ...parsed, topic: event.target.value })} className="rounded-md border border-[var(--br-border)] px-3 py-2" />
            <select value={parsed.level} onChange={(event) => setParsed({ ...parsed, level: event.target.value })} className="rounded-md border border-[var(--br-border)] px-3 py-2">
            ...
            <table className="w-full min-w-[860px] text-left text-sm">
              ...
              <tbody>
                {parsed.questions.map((question, index) => (
                  <tr key={`${question.questionNumber}-${index}`} className={`border-t border-[var(--br-border)] ${question.needsReview ? "bg-coral/5" : ""}`}>
                    <td className="p-2">{question.questionNumber}</td>
                    <td className="p-2">{question.questionType}</td>
                    <td className="p-2">
                      <input value={question.questionText} onChange={(event) => updateQuestion(index, { questionText: event.target.value, needsReview: false })} className="w-full rounded border border-[var(--br-border)] px-2 py-1" />
                    </td>
                    <td className="p-2">
                      <textarea
                    ...
                    <td className="p-2 text-xs text-[var(--br-text-muted)]">
                      {question.questionType === "FILL" ? (
                        <label className="grid gap-1">
                          ...
                          <input
                            ...
                            className="w-24 rounded border border-[var(--br-border)] px-2 py-1"
                          />
                        </label>
                    ...
                    <td className="p-2">
                      <input
                        ...
                        className="w-full rounded border border-[var(--br-border)] px-2 py-1"
                      />
                    </td>
