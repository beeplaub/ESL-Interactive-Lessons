...
function defaultQuestion(type: BuilderQuestion["questionType"]): BuilderQuestion {
  ...
  if (type === "MATCHING") return { id, questionType: type, questionText: "Match the items.", description: "", options: { a_items: ["Word 1", "Word 2"], b_items: ["Meaning A", "Meaning B"], shuffle_options: true }, correctAnswer: [{ a: 1, b: "B" }, { a: 2, b: "A" }], assessment };
  ...
  if (type === "SHADOWING") return { id, questionType: type, questionText: "Listen to the native speaker and repeat the phrase into your microphone.", description: "", options: { audio_url: "", target_text: "Repeat after me.", repeat_count: 3, show_live_match: true, require_all_green: true }, correctAnswer: "Repeat after me.", assessment };
...
function QuestionFields({ question, onChange }: { question: BuilderQuestion; onChange: (patch: Partial<BuilderQuestion>) => void }) {
  ...
  if (question.questionType === "MATCHING") {
    ...
    const setAItem = (index: number, value: string) => onChange({ options: { ...options, a_items: aItems.map((item, itemIndex) => itemIndex === index ? value : item) } as Json });
    const setBItem = (index: number, value: string) => onChange({ options: { ...options, b_items: bItems.map((item, itemIndex) => itemIndex === index ? value : item) } as Json });
    const setAnswer = (index: number, letter: string) => onChange({ correctAnswer: aItems.map((_, itemIndex) => ({ a: itemIndex + 1, b: itemIndex === index ? letter : answers.get(String(itemIndex + 1)) ?? "" })) as Json });
    return (
      <div className="grid gap-3">
    ...
    return (
      <div className="grid gap-3">
        ...
        <p className="text-xs text-[var(--br-text-muted)]">
          Learners will listen to your audio and repeat after you. Groq transcribes each temporary recording and saves the transcript and match result with the attempt; audio is not saved.
        </p>
      </div>
    );
  }
