...
export function LiveVoiceNotes({ sessionId, teacher: initialTeacher = false }: { sessionId: string; teacher?: boolean }) {
  ...
  return <section className="rounded-xl border border-[var(--br-border)] bg-surface p-3 shadow-sm">
    <div className="flex items-center justify-between gap-2"><div className="flex items-center gap-1.5"><Volume2 size={16} className="text-violetglow" /><h2 className="text-sm font-extrabold">Voice notes</h2></div><select value={channel} onChange={(event) => setChannel(event.target.value)} className="rounded-md border border-[var(--br-border)] px-1.5 py-1 text-[10px] font-bold text-[var(--br-text-muted)]"><option value="EVERYONE">Everyone</option><option value="TEACHER">Teacher</option>{ownGroup ? <option value="GROUP">{ownGroup.name}</option> : null}</select></div>
    <div className="mt-3 max-h-44 space-y-2 overflow-y-auto">{data?.messages.length ? data.messages.map((message) => <div key={message.id} className="rounded-lg bg-surface-muted p-2"><div className="flex items-center gap-2"><div className="min-w-0 flex-1"><p className="truncate text-[10px] font-extrabold text-violetglow">{message.sender_name} · {message.channel.toLowerCase()}</p><p className="text-[10px] text-[var(--br-text-muted)]">{message.duration_seconds ? time(message.duration_seconds) : "Voice note"}</p></div>{teacher ? <button type="button" onClick={() => void remove(message.id)} className="rounded p-1 text-[var(--br-text-muted)] hover:bg-red-50 hover:text-[var(--br-danger)]" aria-label="Remove voice note"><Trash2 size={13} /></button> : null}</div><audio controls src={message.url} preload="metadata" className="mt-1.5 h-8 w-full" /></div>) : <p className="py-3 text-center text-xs text-[var(--br-text-muted)]">Voice notes will appear here.</p>}</div>
    <div className="mt-3 flex items-center gap-2 rounded-lg bg-[#F8F6FF] p-2">{recording ? <><button type="button" onClick={stop} className="grid size-9 place-items-center rounded-full bg-[var(--br-danger)] text-on-dark" aria-label="Stop recording"><Square size={14} /></button><span className="text-xs font-extrabold text-[var(--br-danger)]">Recording {time(elapsed)}</span></> : <><button type="button" disabled={sending} onClick={() => void start()} className="grid size-9 place-items-center rounded-full bg-violetglow text-on-dark disabled:opacity-50" aria-label="Record voice note">{sending ? <Loader2 size={15} className="animate-spin" /> : <Mic size={15} />}</button><span className="text-xs font-semibold text-[var(--br-text-muted)]">{sending ? "Sending voice note..." : "Hold a thought? Record a voice note."}</span></>}</div>
