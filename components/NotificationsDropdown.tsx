...
export function NotificationsDropdown({ initialNotifications, mode }: Props) {
  ...
  const visibleNotifications = initialNotifications.filter((item) => !item.notificationId || !deletedNotificationIds.includes(item.notificationId));
  ...
  const toggleRead = (item: NotificationItem) => {
    if (!item.notificationId) return markRead(item);
  ...
  const markAllRead = () => {
    if (!persistentUnreadIds.length || isMarkingAll) return;
  ...
  return (
    <details ref={detailsRef} onToggle={(event) => setIsOpen(event.currentTarget.open)} className="group relative">
      ...
      <div className={panelClass}>
        <div className="flex items-center justify-between gap-3 border-b border-[#ECECF5] px-4 py-3"><div><p className="text-sm font-black text-[#14172B]">Notifications</p><p className="text-xs font-semibold text-[#6E738D]">Latest learning and platform updates</p></div>{persistentUnreadIds.length ? <button type="button" disabled={isMarkingAll} onClick={markAllRead} className="inline-flex shrink-0 items-center gap-1 text-[11px] font-extrabold text-[#6C3BFF] hover:underline disabled:opacity-50"><CheckCheck className="size-3.5" /> Mark all read</button> : null}</div>
