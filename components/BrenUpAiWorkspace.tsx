...
type Health = { connected: boolean; model?: string; harness?: string; reason?: string };
...
export function BrenUpAiWorkspace() {
  ...
  async function send(value = input) {
    ...
    try {
      const response = await fetch("/api/admin/brenup-ai", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message, mode }) });
  ...
  return <main className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
    ...
    <aside className="space-y-4"><section className="rounded-3xl border border-[var(--br-border)] bg-surface p-4 shadow-sm"><div className="flex items-center gap-2"><ShieldCheck className="size-5 text-emerald-600" /><h2 className="font-black text-ink">Safety boundary</h2></div><p className="mt-3 text-sm leading-6 text-[var(--br-text-muted)]">This assistant is read-only. It cannot publish, edit, delete, deploy, or access secrets.</p><p className="mt-3 text-xs font-bold text-emerald-700">No BrenUp records changed</p></section><section className="rounded-3xl border border-[var(--br-border)] bg-surface p-4 shadow-sm"><label className="text-xs font-black uppercase tracking-wide text-[var(--br-text-muted)]">Agent mode<select value={mode} onChange={(event) => setMode(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--br-border)] bg-surface px-3 py-2 text-sm font-bold text-ink"><option value="research">Research</option><option value="audit">Audit</option><option value="review">Review</option><option value="code-review">Code review</option></select></label><div className="mt-4 space-y-2 text-xs text-[var(--br-text-muted)]"><p><strong className="text-ink">Model:</strong> {health.model || "Local Ollama"}</p><p><strong className="text-ink">Harness:</strong> {health.harness || (health.connected ? "Connected" : "Not connected")}</p></div></section><section className="rounded-3xl border border-[var(--br-border)] bg-surface p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="font-black text-ink">Session</h2><button type="button" onClick={() => setMessages([])} className="grid size-8 place-items-center rounded-lg border border-[var(--br-border)] text-[var(--br-text-muted)]"><Plus className="size-4" /></button></div><p className="mt-3 text-sm text-[var(--br-text-muted)]">Start a clean read-only investigation.</p>{lastAnswer ? <button type="button" onClick={() => { void navigator.clipboard?.writeText(lastAnswer); setCopied(true); setTimeout(() => setCopied(false), 1500); }} className="mt-3 inline-flex items-center gap-2 text-xs font-bold text-[var(--br-brand)]"><Copy className="size-3" />{copied ? "Copied" : "Copy last answer"}</button> : null}</section></aside>
  </main>;
}
