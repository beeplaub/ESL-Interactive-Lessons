...
function StrengthBar({ password }: { password: string }) {
  ...
  return (
    <div className="mt-1.5">
      ...
      <p className="mt-0.5 text-[10px] text-[var(--br-text-muted)]">{label}</p>
    </div>
  );
}
...
function ScoreHeader({ score, total, onClose }: { score: number; total: number; onClose: () => void }) {
  ...
  return (
    <div className="bg-dark px-6 py-5 text-on-dark">
      <div className="flex items-start justify-between gap-4">
        ...
        <button type="button" onClick={onClose} aria-label="Close"
          className="mt-0.5 rounded-full p-1.5 text-white/40 hover:bg-white/10 hover:text-on-dark">
          <X size={18} />
        </button>
      </div>
    </div>
  ...
  if (saved) {
    return (
      <>
        ...
        <div className="fixed inset-x-4 top-1/2 z-50 mx-auto max-w-sm -translate-y-1/2 overflow-hidden rounded-2xl bg-surface shadow-2xl">
          <div className="flex flex-col items-center gap-3 px-8 py-10 text-center">
            ...
            <p className="text-sm text-[var(--br-text-muted)]">Your result is now in your account. Taking you there now…</p>
          </div>
        </div>
      </>
    );
  ...
  return (
    <>
      ...
      <div role="dialog" aria-modal="true" aria-label="Save your score"
        className="fixed inset-x-4 top-1/2 z-50 mx-auto max-w-md -translate-y-1/2 overflow-hidden rounded-2xl bg-surface shadow-2xl"
        ...
        <div className="px-6 py-5">
          ...
          <button type="button" disabled={isPending} onClick={signInWithGoogle}
            className="mt-4 flex w-full items-center justify-center gap-3 rounded-md border border-[var(--br-border)] bg-surface px-4 py-2.5 text-sm font-medium text-ink hover:bg-black/[0.03] disabled:opacity-60">
          ...
          <div className="my-3 flex items-center gap-2 text-[10px] uppercase tracking-wide text-[var(--br-text-muted)]">
            <span className="h-px flex-1 bg-black/10" />or<span className="h-px flex-1 bg-black/10" />
          </div>
          <div className="mb-3 grid grid-cols-2 rounded-md border border-[var(--br-border)] bg-black/[0.03] p-0.5 text-xs">
            <button type="button" onClick={() => { setTab("register"); setMessage(null); }}
              className={`rounded py-1.5 font-medium transition-colors ${tab === "register" ? "bg-surface shadow-sm text-ink" : "text-[var(--br-text-muted)] hover:text-[var(--br-text-muted)]"}`}>
              Create account
            </button>
            <button type="button" onClick={() => { setTab("signin"); setMessage(null); }}
              className={`rounded py-1.5 font-medium transition-colors ${tab === "signin" ? "bg-surface shadow-sm text-ink" : "text-[var(--br-text-muted)] hover:text-[var(--br-text-muted)]"}`}>
              Sign in
            </button>
          </div>
          {tab === "register" ? (
            <div className="space-y-2.5">
              <div className="grid grid-cols-2 gap-2">
                <label className="block text-xs font-medium">First name <span className="text-coral">*</span>
                  <input value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="Maria" autoComplete="given-name"
                    className="mt-1 w-full rounded-md border border-[var(--br-border)] px-2.5 py-1.5 text-sm font-normal" />
                </label>
              ...
              <label className="block text-xs font-medium">Password <span className="text-coral">*</span>
                <div className="relative mt-1">
                  <input type={showPw ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                    ...
                    className="w-full rounded-md border border-[var(--br-border)] px-2.5 py-1.5 pr-8 text-sm font-normal" />
                  <button type="button" onClick={() => setShowPw((v) => !v)}
                    className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--br-text-muted)] hover:text-[var(--br-text-muted)]"
              ...
              <button type="button" disabled={isPending} onClick={submitRegister}
                className="w-full rounded-md bg-moss px-4 py-2 text-sm font-semibold text-on-dark disabled:opacity-60">
            ...
            <div className="space-y-2.5">
              ...
              <button type="button" disabled={isPending} onClick={submitSignIn}
                className="w-full rounded-md bg-dark px-4 py-2 text-sm font-semibold text-on-dark disabled:opacity-60">
          ...
          <button type="button" onClick={onDismiss}
            className="mt-4 w-full text-center text-xs text-[var(--br-text-muted)] hover:text-[var(--br-text-muted)]">
