...
import { ArrowLeft, AudioLines, Check, Copy, ExternalLink, Library, Loader2, Lock, Mic2, Music2, Pause, Play, RefreshCw, Save, Sparkles, Unlock, Volume2 } from "lucide-react";
...
export function VoiceoverStudio({ canUse, accessMessage, voices, styles, paces, recentVoiceovers, lessonContext }: {
  ...
  useEffect(() => {
    if (!voiceSettings.ready || hydratedSettings.current || !voiceSettings.preferences) return;
    ...
    setStyle(voiceSettings.preferences.style);
    setPace(voiceSettings.preferences.pace);
  ...
  async function generate() {
    ...
    try {
      ...
      const data = await readJson(await fetch("/api/creator-tools/voiceover", {
        ...
        body: JSON.stringify({ title, script, voiceName, languageCode, style, pace, provider }),
      }));
      ...
      <section className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(340px,0.9fr)]">
        <div className="min-w-0 space-y-4 rounded-2xl border border-[var(--br-border)] bg-surface p-4 shadow-sm sm:p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-sm font-semibold">Language<select disabled={voiceSettings.preferences.locked} value={languageCode} onChange={(event) => { setLanguageCode(event.target.value); voiceSettings.update({ languageCode: event.target.value }); }} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal disabled:cursor-not-allowed disabled:bg-surface-muted">{languageOptions.map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            <div className="min-w-0"><div className="flex items-end gap-2"><label className="min-w-0 flex-1 text-sm font-semibold">Voice<select disabled={voiceSettings.preferences.locked} value={voiceName} onChange={(event) => { setVoiceName(event.target.value); voiceSettings.update({ voiceName: event.target.value }); }} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal disabled:cursor-not-allowed disabled:bg-surface-muted"><optgroup label="Female voices">{displayVoices.filter((voice) => voice.presentation === "Female").map((voice) => <option key={voice.name} value={voice.name}>{voice.label} · {voice.description}</option>)}</optgroup><optgroup label="Male voices">{displayVoices.filter((voice) => voice.presentation === "Male").map((voice) => <option key={voice.name} value={voice.name}>{voice.label} · {voice.description}</option>)}</optgroup></select></label><button type="button" onClick={toggleVoiceSample} disabled={!selectedVoice?.sampleUrl} className="mb-0.5 grid size-11 shrink-0 place-items-center rounded-lg border border-[var(--br-brand)]/25 bg-[var(--br-brand)]/5 text-[var(--br-brand)] transition hover:bg-[var(--br-brand)]/10 disabled:opacity-50" title={`Listen to ${selectedVoice?.label ?? "voice"} sample`} aria-label={`Listen to ${selectedVoice?.label ?? "voice"} sample`}>{samplePlaying ? <Pause size={17} fill="currentColor" /> : <Play size={17} fill="currentColor" />}</button></div><audio ref={sampleAudioRef} key={selectedVoice?.name} src={selectedVoice?.sampleUrl} preload="metadata" className="hidden" onEnded={() => setSamplePlaying(false)} onPause={() => setSamplePlaying(false)} /></div>
            <label className="text-sm font-semibold">Engine<select disabled={voiceSettings.preferences.locked} value={provider} onChange={(event) => { const next = event.target.value as "auto" | "kokoro" | "google"; setProvider(next); voiceSettings.update({ provider: next }); }} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal disabled:cursor-not-allowed disabled:bg-surface-muted"><option value="auto">Auto · best fit</option><option value="kokoro" disabled={!kokoroAvailable}>Kokoro · English only</option><option value="google">Gemini · languages & styles</option></select></label>
            <label className="text-sm font-semibold">Delivery style<select disabled={voiceSettings.preferences.locked} value={style} onChange={(event) => { setStyle(event.target.value); voiceSettings.update({ style: event.target.value }); }} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal disabled:cursor-not-allowed disabled:bg-surface-muted">{availableStyles.map((value) => <option key={value}>{value}</option>)}</select></label>
            <label className="text-sm font-semibold">Pace<select disabled={voiceSettings.preferences.locked} value={pace} onChange={(event) => { setPace(event.target.value); voiceSettings.update({ pace: event.target.value }); }} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal disabled:cursor-not-allowed disabled:bg-surface-muted">{paces.map((value) => <option key={value}>{value}</option>)}</select></label>
          </div>
          <div className="flex items-center justify-between gap-3 rounded-lg border border-[var(--br-border)] bg-surface-muted px-3 py-2"><span className="text-xs font-semibold text-[var(--br-text-muted)]">{voiceSettings.preferences.locked ? "Settings locked" : "Settings editable"}</span><button type="button" onClick={() => void toggleSettingsLock()} disabled={voiceSettings.saving} className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold transition ${voiceSettings.preferences.locked ? "bg-[var(--br-brand)]/10 text-[var(--br-brand)]" : "bg-black/5 text-[var(--br-text-muted)] hover:bg-black/10"}`} title={voiceSettings.preferences.locked ? "Unlock voice settings" : "Lock voice settings"}>{voiceSettings.preferences.locked ? <Lock size={12} /> : <Unlock size={12} />}{voiceSettings.preferences.locked ? "Locked" : "Lock"}</button></div>
          {voiceSettings.error ? <p className="text-xs text-[var(--br-danger)]">{voiceSettings.error}</p> : null}
          <p className="rounded-lg bg-surface-muted px-3 py-2 text-xs text-[var(--br-text-muted)]"><strong className="text-ink">{selectedVoice?.label}</strong> uses {effectiveProvider === "kokoro" ? <><strong className="text-ink">Kokoro {selectedVoice?.kokoroVoice}</strong> for natural English. Pace is applied locally; delivery style is Natural.</> : <><strong className="text-ink">Gemini</strong> for {languageCode.startsWith("en") ? "expressive delivery" : "this language"}. Delivery style and pace are applied by the model.</>} Samples are stored on R2 and never use generation quota.</p>
