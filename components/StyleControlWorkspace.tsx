...
import { Activity, Check, Download, Eye, History, LayoutPanelTop, Loader2, Map, Palette, RotateCcw, ShieldCheck, Sparkles, Upload } from "lucide-react";
...
export function StyleControlWorkspace({ initial, revision, revisions }: { initial: PlatformStyleSettings; revision: number; revisions: Array<{ id: string; revision: number; settings: PlatformStyleSettings; label: string | null; createdAt: string }> }) {
  ...
  const comparisonRevision = useMemo(() => revisions.find((item) => item.id === comparisonRevisionId) ?? null, [comparisonRevisionId, revisions]);
  ...
  function save() {
    ...
    form.set("revisionLabel", revisionLabel);
  ...
  return <main className="min-w-0 space-y-5" style={variables}>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_350px]">
      <section className="br-admin-card rounded-[var(--br-radius)] p-4 sm:p-5">
        ...
        <div className="mt-6 flex flex-wrap items-end gap-3"><label className="min-w-[180px] flex-1 text-xs font-bold text-[var(--br-text-muted)]">Snapshot name<input value={revisionLabel} maxLength={80} onChange={(event) => setRevisionLabel(event.target.value)} placeholder="For example: August course refresh" className="mt-1.5 w-full rounded-md border border-[var(--br-border)] bg-[var(--br-surface)] px-3 py-2 text-sm font-medium text-[var(--br-text)]" /></label><button type="button" onClick={save} disabled={pending} className="br-button-admin inline-flex items-center gap-2 px-4 py-2.5 text-sm font-bold disabled:opacity-60">{pending ? <Loader2 size={16} className="animate-spin" /> : saved ? <Check size={16} /> : <ShieldCheck size={16} />}{saved ? "Published" : "Publish theme"}</button><p className="max-w-xs pb-1 text-xs text-[var(--br-text-muted)]">Publishing creates a restorable revision.</p></div>
      </section>
      <aside className="space-y-4"><PreviewGallery settings={settings} /><section className="br-admin-card rounded-[var(--br-radius)] p-4"><div className="flex items-center gap-2"><ShieldCheck size={17} className="text-[var(--br-success)]" /><h2 className="font-bold text-[var(--br-text)]">Contrast checks</h2></div>{contrastChecks.map(([label, value]) => <ContrastRow key={label} label={label} value={value} />)}<p className="mt-3 text-xs leading-5 text-[var(--br-text-muted)]">Normal body text should reach 4.5:1. Choose stronger colours before publishing a warning state.</p></section><section className="br-admin-card rounded-[var(--br-radius)] p-4"><div className="flex items-center gap-2"><History size={17} className="text-[var(--br-brand)]" /><h2 className="font-bold text-[var(--br-text)]">Revision history</h2></div><label className="mt-3 block text-xs font-semibold text-[var(--br-text-muted)]">Compare draft with<select value={comparisonRevisionId} onChange={(event) => setComparisonRevisionId(event.target.value)} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-[var(--br-surface)] px-3 py-2 text-xs font-semibold text-[var(--br-text)]"><option value="">Choose a saved revision</option>{revisions.map((item) => <option key={item.id} value={item.id}>{item.label || `Revision ${item.revision}`} · #{item.revision}</option>)}</select></label>{comparisonRevision ? <RevisionComparison draft={settings} saved={comparisonRevision.settings} label={comparisonRevision.label || `Revision ${comparisonRevision.revision}`} /> : null}<div className="mt-3 space-y-2">{revisions.map((item) => <div key={item.id} className="flex items-center gap-2 rounded-lg bg-[var(--br-surface-muted)] px-3 py-2"><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold text-[var(--br-text)]">{item.label || `Revision ${item.revision}`}</p><p className="text-[10px] text-[var(--br-text-muted)]">Revision {item.revision} · {new Date(item.createdAt).toLocaleString()}</p></div>{item.revision !== revision ? <button type="button" disabled={pending} onClick={() => restore(item.id)} className="text-xs font-bold text-[var(--br-brand)] disabled:opacity-50">Restore</button> : <span className="text-[10px] font-bold text-[var(--br-success)]">Current</span>}</div>)}{!revisions.length ? <p className="text-xs text-[var(--br-text-muted)]">Your first published change creates a restorable revision.</p> : null}</div></section></aside>
    </div>
  </main>;
}
...
function PreviewGallery({ settings }: { settings: PlatformStyleSettings }) { const vars = platformStyleVariables(settings); return <section className="br-admin-card rounded-[var(--br-radius)] p-4" style={vars}><div className="flex items-center gap-2"><Eye size={17} className="text-[var(--br-brand)]" /><h2 className="font-bold text-[var(--br-text)]">Live preview gallery</h2></div><p className="mt-1 text-xs text-[var(--br-text-muted)]">Learner, creator, activity, and live-class surfaces respond to this draft.</p><div className="mt-4 grid gap-3"><PreviewSurface icon={<Sparkles size={14} />} title="Learner card"><p className="text-xs font-bold text-[var(--br-text-muted)]">Vocabulary practice</p><p className="mt-1 text-sm font-bold text-[var(--br-text)]">Speak with confidence</p><div className="mt-3 flex items-center gap-2"><button className="rounded-lg bg-[var(--br-action)] px-3 py-2 text-xs font-bold text-[var(--br-text-on-dark)]">Continue</button><span className="rounded-full bg-[var(--br-success)] px-2 py-1 text-[10px] font-bold text-[var(--br-text-on-dark)]">Complete</span></div></PreviewSurface><PreviewSurface icon={<LayoutPanelTop size={14} />} title="Creator table"><div className="mt-1 grid grid-cols-[1fr_auto] gap-2 rounded-md border border-[var(--br-border)] bg-[var(--br-surface-muted)] px-2 py-2 text-[10px]"><span className="font-bold text-[var(--br-text)]">Slide 04 · Reading</span><span className="font-bold text-[var(--br-brand)]">Edit</span></div></PreviewSurface><PreviewSurface icon={<Activity size={14} />} title="Activity feedback"><div className="mt-1 rounded-md bg-[var(--br-success-soft)] px-2 py-2 text-[10px] font-bold text-[var(--br-success)]">Correct answer · 2 points earned</div></PreviewSurface><PreviewSurface icon={<History size={14} />} title="Live-class toolbar"><div className="mt-1 flex gap-1.5 rounded-md bg-[var(--br-dark-card)] p-2"><span className="rounded bg-[var(--br-surface)] px-2 py-1 text-[10px] font-bold text-[var(--br-brand)]">Poll</span><span className="rounded bg-[var(--br-action)] px-2 py-1 text-[10px] font-bold text-[var(--br-text-on-dark)]">End class</span></div></PreviewSurface></div></section>; }
function PreviewSurface({ icon, title, children }: { icon: ReactNode; title: string; children: ReactNode }) { return <div className="overflow-hidden rounded-xl border border-[var(--br-border)]"><div className="flex items-center gap-2 bg-[var(--br-dark-card)] px-3 py-2 text-[var(--br-text-on-dark)]">{icon}<span className="text-xs font-bold">{title}</span></div><div className="bg-[var(--br-canvas)] p-3">{children}</div></div>; }
function RevisionComparison({ draft, saved, label }: { draft: PlatformStyleSettings; saved: PlatformStyleSettings; label: string }) { const changed = (Object.keys(draft) as Array<keyof PlatformStyleSettings>).filter((key) => draft[key] !== saved[key]).filter((key) => !["schemaVersion"].includes(key)); return <div className="mt-3 rounded-lg border border-[var(--br-border)] bg-[var(--br-surface-muted)] p-3"><p className="text-xs font-bold text-[var(--br-text)]">Draft compared with {label}</p><p className="mt-1 text-xs text-[var(--br-text-muted)]">{changed.length ? `${changed.length} setting${changed.length === 1 ? "" : "s"} changed.` : "No draft differences."}</p>{changed.length ? <div className="mt-2 flex flex-wrap gap-1">{changed.slice(0, 8).map((key) => <span key={key} className="rounded-full border border-[var(--br-border)] bg-[var(--br-surface)] px-2 py-1 text-[10px] font-bold text-[var(--br-brand)]">{key.replace(/([A-Z])/g, " $1").trim()}</span>)}{changed.length > 8 ? <span className="px-1 py-1 text-[10px] font-bold text-[var(--br-text-muted)]">+{changed.length - 8} more</span> : null}</div> : null}</div>; }
