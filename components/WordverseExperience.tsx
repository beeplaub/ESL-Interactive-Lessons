...
import {
  ...
  Info, LocateFixed, Pause, RotateCcw, Search, Sparkles, Star, Volume2, Zap,
} from "lucide-react";
...
export function WordverseExperience({ topics, words, relationships, progress }: Props) {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050a16] text-white">
      <div className="relative mx-auto min-h-screen max-w-[1700px] overflow-hidden border-x border-white/10 bg-[radial-gradient(circle_at_52%_42%,rgba(35,72,130,.28),transparent_32%),radial-gradient(circle_at_18%_84%,rgba(108,59,255,.12),transparent_25%),#050a16]">
        ...
        <header className="relative z-10 flex flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-[#071020]/75 px-4 py-4 backdrop-blur-xl sm:px-7">
          <div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl border border-cyan-300/30 bg-cyan-300/10 text-cyan-200 shadow-[0_0_32px_rgba(94,231,255,.18)]"><Globe2 size={22} /></div><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-cyan-200/70">BrenUp</p><h1 className="text-xl font-black tracking-tight sm:text-2xl">Wordverse</h1></div></div>
          <div className="flex min-w-[260px] flex-1 items-center gap-2 sm:max-w-xl"><label className="flex flex-1 items-center gap-2 rounded-2xl border border-white/10 bg-white/[.05] px-3 py-2.5 text-sm text-white/60 focus-within:border-cyan-300/60"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search the universe…" className="min-w-0 flex-1 bg-transparent text-white outline-none placeholder:text-white/35" aria-label="Search vocabulary" /></label><button type="button" onClick={() => setShowFilters((open) => !open)} aria-expanded={showFilters} className="grid size-11 place-items-center rounded-2xl border border-white/10 bg-white/[.05] text-white/60 transition hover:border-cyan-300/50 hover:text-cyan-200"><Filter size={18} /></button></div>
          <div className="flex items-center gap-2 rounded-2xl border border-cyan-300/20 bg-cyan-300/[.07] px-3 py-2 text-xs font-bold text-cyan-100"><Zap size={15} /> {words.length.toLocaleString()} words mapped</div>
        </header>
        ...
        <div className="relative z-10 flex min-h-[calc(100vh-82px)] flex-col lg:flex-row">
          <section className="relative min-h-[650px] flex-1 overflow-hidden p-4 sm:p-7 lg:min-h-[calc(100vh-82px)]">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-cyan-200/65"><Compass size={14} /> {view === "universe" ? "Universe of Vocabulary" : "Solar System of a Word"}</p><p className="mt-1 max-w-xl text-sm text-white/50">{view === "universe" ? "Travel through meaning, memory, and mastery." : "Orbit every dimension of the word and discover its gravity."}</p></div><div className="flex rounded-xl border border-white/10 bg-white/[.04] p-1 text-xs font-bold"><button type="button" onClick={() => setView("universe")} className={`rounded-lg px-3 py-2 ${view === "universe" ? "bg-cyan-300/15 text-cyan-100" : "text-white/50"}`}>Universe</button><button type="button" onClick={() => setView("solar")} className={`rounded-lg px-3 py-2 ${view === "solar" ? "bg-violet-300/15 text-violet-100" : "text-white/50"}`}>Solar System</button></div></div>
            {view === "universe" ? <UniverseMap words={filteredWords} allWords={words} relationships={relationships} selectedId={selected.id} progressMap={progressMap} topicMap={topicMap} onSelect={openWord} /> : <SolarSystem word={selected} words={words} relationships={selectedRelationships} topic={topicMap.get(selected.topic_id ?? "")} progress={progressMap.get(selected.id)} onSelect={openWord} />}
          </section>
          <WordPanel word={selected} words={words} topic={topicMap.get(selected.topic_id ?? "")} progress={progressMap.get(selected.id)} isPending={isPending} onBack={() => setView("universe")} onAction={progressAction} onOpenWord={openWord} onPlay={playWord} />
        </div>
      </div>
    </main>
  );
...
function UniverseMap({ words, allWords, relationships, selectedId, progressMap, topicMap, onSelect }: { words: WordverseWord[]; allWords: WordverseWord[]; relationships: WordverseRelationship[]; selectedId: string; progressMap: Map<string, WordverseProgress>; topicMap: Map<string, WordverseTopic>; onSelect: (id: string) => void }) {
  const positions = new Map(allWords.map((word, index) => [word.id, nodePositions[index % nodePositions.length]]));
  ...
  return <div className="relative h-[560px] overflow-hidden rounded-[28px] border border-cyan-200/15 bg-[#06101e]/70 shadow-[inset_0_0_100px_rgba(20,65,120,.17),0_25px_90px_rgba(0,0,0,.25)] sm:h-[calc(100vh-235px)] sm:min-h-[600px]">
    ...
    {words.map((word, index) => { const position = positions.get(word.id) ?? nodePositions[index % nodePositions.length]; const item = progressMap.get(word.id); const color = stateColors[item?.state ?? "DISCOVERED"]; const topic = topicMap.get(word.topic_id ?? ""); const isSelected = word.id === selectedId; return <button key={word.id} type="button" onClick={() => onSelect(word.id)} aria-label={`Open ${word.word}, ${word.definition}`} className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full text-center transition duration-500 hover:scale-125 focus:outline-none focus:ring-2 focus:ring-cyan-200 ${isSelected ? "z-10 scale-125" : ""}`} style={{ left: `${position[0]}%`, top: `${position[1]}%`, color }}><span className="absolute inset-[-8px] rounded-full border opacity-60 motion-safe:animate-[pulse_4s_ease-in-out_infinite]" style={{ borderColor: color, boxShadow: `0 0 18px ${color}55` }} /><span className={`grid size-9 place-items-center rounded-full border bg-[#091828]/90 text-[9px] font-black shadow-[0_0_20px_rgba(94,231,255,.12)] sm:size-12 sm:text-[10px] ${isSelected ? "border-cyan-100 shadow-[0_0_32px_rgba(94,231,255,.55)]" : "border-white/20"}`} style={{ borderColor: isSelected ? "#d8fbff" : `${color}99` }}>{word.word.slice(0, 1).toUpperCase()}</span><span className="mt-2 block max-w-24 truncate text-[10px] font-bold text-white/80 sm:text-xs">{word.word}</span>{topic ? <span className="mt-0.5 hidden text-[8px] text-white/35 sm:block">{topic.name}</span> : null}</button>; })}
...
function WordPanel({ word, topic, progress, isPending, words, onBack, onAction, onOpenWord, onPlay }: { word: WordverseWord; topic?: WordverseTopic; progress?: WordverseProgress; isPending: boolean; words: WordverseWord[]; onBack: () => void; onAction: (intent: "toggle_saved" | "familiar" | "review" | "confidence", confidence?: number) => void; onOpenWord: (id: string) => void; onPlay: () => void }) {
