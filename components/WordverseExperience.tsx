      ...
      <div className="relative mx-auto flex h-full min-h-0 overflow-hidden border-x border-white/10 bg-[radial-gradient(circle_at_48%_42%,rgba(30,74,137,.24),transparent_34%),radial-gradient(circle_at_20%_82%,rgba(91,62,220,.12),transparent_27%),#030811]">
        <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <header className="relative z-30 flex shrink-0 flex-wrap items-center gap-2 border-b border-white/10 bg-[#071020]/90 p-3">
            ...
            {isWordView ? <button type="button" onClick={() => setSidebarOpen(open => !open)} aria-label={sidebarOpen ? "Hide word details" : "Show word details"} aria-expanded={sidebarOpen} title={sidebarOpen ? "Hide word details" : "Show word details"} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-cyan-300/30 px-3 text-xs font-semibold text-cyan-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200"><Info size={18} /><span>Details</span></button> : null}
        ...
        {isWordView && sidebarOpen ? <WordPanelContainer message={saveMessage} word={selected} words={words} topic={topicMap.get(selected.topic_id ?? "")} progress={sceneProgress.get(selected.id)} isPending={isPending} onClose={() => setSidebarOpen(false)} onBack={backToPrevious} onAction={progressAction} onPractice={startPractice} onOpenWord={openWord} onPlay={playWord} /> : null}
