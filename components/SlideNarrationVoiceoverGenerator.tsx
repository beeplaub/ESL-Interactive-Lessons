...
import { AudioLines, Check, Loader2, Mic2, Pause, Play, RefreshCw, Save, Sparkles, Volume2, X } from "lucide-react";
...
export function SlideNarrationVoiceoverGenerator({ lessonId, slideId, onAttached, onClose }: Props) {
  const [title, setTitle] = useState("Slide narration");
  ...
  async function saveAndAttach() {
    if (!preview || !title.trim()) {
      setError("Add a clear title, generate a preview, then save it.");
      return;
    }
    ...
    try {
      ...
      if (!preview.saved) {
        const saved = await readJson(await fetch("/api/creator-tools/voiceover/save", {
          method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ generationId, title }),
        }));
  ...
  return <section className="w-full max-w-4xl overflow-hidden rounded-2xl border border-[var(--br-border)] bg-surface shadow-2xl">
    ...
    <div className="grid max-h-[min(76vh,760px)] gap-5 overflow-y-auto p-4 sm:p-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
      <div className="min-w-0 space-y-3.5">
        <label className="block text-sm font-bold text-ink">Asset title<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Slide 4 warm-up narration" className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal" /></label>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="min-w-0"><div className="flex items-end gap-2"><label className="min-w-0 flex-1 text-sm font-bold text-ink">Voice<select value={voiceName} onChange={(event) => setVoiceName(event.target.value)} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal"><optgroup label="Female voices">{displayVoices.filter((voice) => voice.presentation === "Female").map((voice) => <option key={voice.name} value={voice.name}>{voice.label} · {voice.description}</option>)}</optgroup><optgroup label="Male voices">{displayVoices.filter((voice) => voice.presentation === "Male").map((voice) => <option key={voice.name} value={voice.name}>{voice.label} · {voice.description}</option>)}</optgroup></select></label><button type="button" onClick={() => void toggleSample()} disabled={!voiceSampleUrl(selectedVoice)} className="mb-0.5 grid size-11 shrink-0 place-items-center rounded-lg border border-[var(--br-brand)]/25 bg-[var(--br-brand)]/5 text-[var(--br-brand)] disabled:opacity-45" title="Listen to voice sample">{samplePlaying ? <Pause size={17} fill="currentColor" /> : <Play size={17} fill="currentColor" />}</button></div><audio ref={sampleRef} key={selectedVoice?.name} src={voiceSampleUrl(selectedVoice)} className="hidden" onEnded={() => setSamplePlaying(false)} onPause={() => setSamplePlaying(false)} /></div>
          <label className="text-sm font-bold text-ink">Engine<select value={provider} onChange={(event) => setProvider(event.target.value as "auto" | "kokoro" | "google")} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal"><option value="auto">Auto · best fit</option><option value="kokoro" disabled={!kokoroAvailable}>Kokoro · English only</option><option value="google">Gemini · languages & styles</option></select></label>
          <label className="text-sm font-bold text-ink">Delivery style<select value={style} onChange={(event) => setStyle(event.target.value as (typeof VOICEOVER_STYLES)[number])} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal">{availableStyles.map((value) => <option key={value}>{value}</option>)}</select></label>
          <label className="text-sm font-bold text-ink">Pace<select value={pace} onChange={(event) => setPace(event.target.value as (typeof VOICEOVER_PACES)[number])} className="mt-1.5 w-full rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2.5 font-normal">{VOICEOVER_PACES.map((value) => <option key={value}>{value}</option>)}</select></label>
        </div>
      ...
      <aside className="min-w-0 space-y-3.5">
        <section className="overflow-hidden rounded-xl bg-[var(--br-dark-card)] p-4 text-on-dark shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.14em] text-white/50">Voice preview</p><h3 className="mt-1 text-base font-black">{title || "Untitled narration"}</h3></div><span className="grid size-9 place-items-center rounded-full bg-white/10"><AudioLines size={17} /></span></div>{preview ? <><audio ref={previewRef} controls src={preview.url} className="mt-5 w-full" /><p className="mt-3 text-xs text-white/60">{voiceName} · {style} · {Math.round(preview.durationSeconds || 0)} sec</p></> : <div className="mt-5 grid min-h-36 place-items-center rounded-lg border border-dashed border-white/20 text-center"><div><Volume2 size={25} className="mx-auto text-white/35" /><p className="mt-2 text-xs text-white/55">Your generated audio will appear here.</p></div></div>}</section>
