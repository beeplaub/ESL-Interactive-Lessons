...
function BlockFields({ blockType, content, lessonId, blockId }: { blockType: string; content: Json; lessonId: string; blockId: string }) {
  ...
  const [contrastPairs, setContrastPairs] = useState(() => {
    ...
    return (pairs.length ? pairs : [{}]).map((pair) => ({
      ...
      rightTerm: asString(pair.right_term), rightMeaning: asString(pair.right_meaning), rightPattern: asString(pair.right_pattern), rightExamples: Array.isArray(pair.right_examples) ? pair.right_examples.map(String).join("\n") : "",
  ...
  if (blockType === "CONTRAST_PAIR") {
    ...
    const blankPair = { title: "", context: "", leftTerm: "", leftMeaning: "", leftPattern: "", leftExamples: "", rightTerm: "", rightMeaning: "", rightPattern: "", rightExamples: "", keyDifference: "", commonMistake: "" };
    return (
      <div className="grid gap-3">
        ...
        <input type="hidden" name="pairs_json" value={JSON.stringify(contrastPairs.map((pair) => ({ title: pair.title, context: pair.context || null, left_term: pair.leftTerm, left_meaning: pair.leftMeaning, left_pattern: pair.leftPattern || null, left_examples: lines(pair.leftExamples), right_term: pair.rightTerm, right_meaning: pair.rightMeaning, right_pattern: pair.rightPattern || null, right_examples: lines(pair.rightExamples), key_difference: pair.keyDifference || null, common_mistake: pair.commonMistake || null })))} />
        <div className="grid gap-3">
