...
function MoveSlideModal({
  ...
  const isValid = !isNaN(targetNum) && targetNum >= 1 && targetNum <= totalSlides && targetNum !== slide.slide_number;

  function submit() {
    if (isValid) onMove(targetNum - 1);
    onClose();
  }
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 px-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-2xl">
        ...
        <p className="mt-2 text-sm text-black/55">
          <span className="font-medium">&ldquo;{slide.title}&rdquo;</span> is currently at position {slide.slide_number} of {totalSlides}.
        </p>
        <div className="mt-4">
          <label className="text-sm font-medium">
            Move to position
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {[1, Math.ceil(totalSlides / 2), totalSlides]
            .filter((n, i, arr) => arr.indexOf(n) === i && n !== slide.slide_number)
            .map((n) => (
              <button key={n} type="button" onClick={() => setTarget(String(n))} className="rounded-md border border-black/10 px-3 py-1 text-xs font-medium hover:bg-black/5">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded-md border border-black/15 px-4 py-2 text-sm hover:bg-black/5">Cancel</button>
          <button type="button" onClick={submit} disabled={!isValid} className="rounded-md bg-moss px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">
            Move slide
          </button>
        </div>
      </div>
...
export function LessonBuilderWorkspace({ lesson, slides: serverSlides, blocks, activities }: Props) {
  ...
  // Sync when slides are added or deleted (count changes)
  useEffect(() => {
  ...
  // Core reorder — optimistic update then persist
  function applyReorder(newOrder: Slide[]) {
    ...
    startReorderTransition(async () => {
      await reorderBuilderSlides(lesson.id, renumbered.map((s) => s.id));
    });
  }

  function handleDragStart(slideId: string) { setDraggedSlideId(slideId); }
  ...
  function handleDragEnd() { setDraggedSlideId(null); setDragOverSlideId(null); }
  ...
  return (
    ...
    >
      ...
      {moveSlide && (
        <MoveSlideModal slide={moveSlide} totalSlides={slides.length} onMove={handleMoveToPosition} onClose={() => setMoveSlide(null)} />
      )}
      ...
      <section className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)] lg:gap-5">
        ...
        <section className="min-w-0 rounded-xl border border-black/10 bg-white p-3 shadow-sm sm:p-4">
          ...
          {/* Slide strip */}
          <div className="mt-3">
            <p className="mb-1.5 text-[11px] text-black/35">Drag to reorder · Click to select</p>
              ...
              {slides.map((slide, index) => (
                <div key={slide.id} className="flex shrink-0 items-center">
                  <div
                    ...
                    className={`min-w-44 cursor-grab rounded-lg border px-3 py-2 text-left text-sm transition active:cursor-grabbing
                      ${slide.id === selectedSlide?.id ? "border-moss bg-moss/10" : "border-black/10 bg-white hover:bg-black/[0.03]"}
                      ${dragOverSlideId === slide.id && draggedSlideId !== slide.id ? "scale-[1.02] border-moss/50 bg-moss/5" : ""}
                  ...
                  <button type="button" onClick={() => setAddAfter(slide.slide_number)} title={`Add slide after ${index + 1}`} className="mx-1 flex size-7 shrink-0 items-center justify-center rounded-full border border-dashed border-black/20 text-black/30 transition hover:border-moss hover:bg-moss/5 hover:text-moss">
                    <Plus size={13} />
                  </button>
                </div>
              ))}
...
function SelectedSlideEditor({
  ...
  lessonId: string; slide: Slide; slideIndex: number; slideCount: number;
  blocks: LessonBlock[]; activity: Activity | null;
    ...
    <div className="grid min-w-0 gap-5 xl:grid-cols-2">
      <section className="min-w-0">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => onMoveUpDown(slide.id, "up")} disabled={slideIndex === 0} className="rounded-md border border-black/15 p-2 hover:bg-black/5 disabled:opacity-35" aria-label="Move up"><ArrowUp size={15} /></button>
            <button type="button" onClick={() => onMoveUpDown(slide.id, "down")} disabled={slideIndex === slideCount - 1} className="rounded-md border border-black/15 p-2 hover:bg-black/5 disabled:opacity-35" aria-label="Move down"><ArrowDown size={15} /></button>
            <button type="button" onClick={onMoveToPosition} className="rounded-md border border-black/15 p-2 hover:bg-black/5" aria-label="Move to position" title="Move to position"><Move size={15} /></button>
...
function BlockFields({ blockType, content, lessonId }: { blockType: string; content: Json; lessonId: string }) {
  ...
  if (blockType === "HEADING") return <label className="text-sm">Heading text<textarea name="text" rows={2} defaultValue={asString(data.text)} className="mt-1 w-full rounded-md border border-black/15 px-3 py-2" /></label>;
  if (blockType === "TEXT") return <label className="text-sm">Body text<textarea name="body" rows={4} defaultValue={asString(data.body ?? data.text)} className="mt-1 w-full rounded-md border border-black/15 px-3 py-2" /></label>;
  if (blockType === "BULLETS") return <label className="text-sm">Bullet points <span className="font-normal text-black/45">(one per line)</span><textarea name="items" rows={5} defaultValue={lines(data.items)} className="mt-1 w-full rounded-md border border-black/15 px-3 py-2" /></label>;
  ...
  if (blockType === "DIALOGUE") {
    const turnsText = Array.isArray(data.turns)
      ...
      : "";
    return <label className="text-sm">Dialogue lines <span className="font-normal text-black/45">(Speaker: Line — one per line)</span><textarea name="turns" rows={6} defaultValue={turnsText} className="mt-1 w-full rounded-md border border-black/15 px-3 py-2" /></label>;
  }
