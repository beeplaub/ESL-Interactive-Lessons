  ...
  if (blockType === "CONTRAST_PAIR") {
    ...
    return (
      <div className="grid gap-3">
        ...
        <input type="hidden" name="pairs_json" value={JSON.stringify(contrastPairs.map((pair) => ({ title: pair.title, context: pair.context || null, left_term: pair.leftTerm, left_meaning: pair.leftMeaning, left_pattern: pair.leftPattern || null, left_examples: lines(pair.leftExamples), left_color: pair.leftColor, right_term: pair.rightTerm, right_meaning: pair.rightMeaning, right_pattern: pair.rightPattern || null, right_examples: lines(pair.rightExamples), right_color: pair.rightColor, key_difference: pair.keyDifference || null, common_mistake: pair.commonMistake || null })))} />
        <div className="grid gap-3">
          {contrastPairs.map((pair, index) => <div key={index} className="rounded-xl border border-[var(--br-brand)]/20 bg-[var(--br-brand-soft)]/25 p-3 sm:p-4">
            ...
            <div className="grid gap-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {(["left", "right"] as const).map((side) => <div key={side} className="rounded-lg border border-[var(--br-info)]/25 bg-surface p-3"><p className="mb-2 text-xs font-black uppercase tracking-wide text-[var(--br-info)]">{side === "left" ? "First term" : "Second term"}</p><div className="grid gap-2"><label className="text-sm">Term<input value={side === "left" ? pair.leftTerm : pair.rightTerm} onChange={(event) => updatePair(index, side === "left" ? "leftTerm" : "rightTerm", event.target.value)} className="mt-1 w-full rounded-md border border-[var(--br-border)] px-3 py-2" /></label><label className="text-sm">Card color<select value={side === "left" ? pair.leftColor : pair.rightColor} onChange={(event) => updatePair(index, side === "left" ? "leftColor" : "rightColor", event.target.value)} className="mt-1 w-full rounded-md border border-[var(--br-border)] bg-surface px-3 py-2"><option value="var(--br-brand)">Moss</option><option value="var(--br-info)">Blue</option><option value="var(--br-dark-card)">Midnight</option><option value="var(--br-action)">Coral</option><option value="var(--br-success)">Green</option><option value="var(--br-chart-primary)">Violet</option></select></label><label className="text-sm">Meaning<textarea value={side === "left" ? pair.leftMeaning : pair.rightMeaning} onChange={(event) => updatePair(index, side === "left" ? "leftMeaning" : "rightMeaning", event.target.value)} rows={2} className="mt-1 w-full rounded-md border border-[var(--br-border)] px-3 py-2" /></label><label className="text-sm">Usage pattern <span className="font-normal text-[var(--br-text-muted)]">(optional)</span><input value={side === "left" ? pair.leftPattern : pair.rightPattern} onChange={(event) => updatePair(index, side === "left" ? "leftPattern" : "rightPattern", event.target.value)} className="mt-1 w-full rounded-md border border-[var(--br-border)] px-3 py-2" /></label><label className="text-sm">Examples <span className="font-normal text-[var(--br-text-muted)]">(one per line)</span><textarea value={side === "left" ? pair.leftExamples : pair.rightExamples} onChange={(event) => updatePair(index, side === "left" ? "leftExamples" : "rightExamples", event.target.value)} rows={3} className="mt-1 w-full rounded-md border border-[var(--br-border)] px-3 py-2" /></label></div></div>)}
              </div>
