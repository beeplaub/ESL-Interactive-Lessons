...
function PreviewActivitySequence({ activities, onNextSlide }: { activities: Activity[]; onNextSlide: () => void }) {
  ...
  return <div className="space-y-3"><div className="flex items-center gap-1 rounded-xl bg-[var(--br-canvas-elevated)] p-1" role="tablist" aria-label="Activities on this slide">{activities.map((item, index) => <button key={item.id} type="button" role="tab" aria-selected={index === activityIndex} onClick={() => setActivityIndex(index)} className={`min-w-0 flex-1 rounded-lg px-2.5 py-1.5 text-[11px] font-extrabold transition sm:px-3 ${index === activityIndex ? "bg-[var(--br-chart-secondary)] text-on-dark shadow-sm" : "text-[var(--br-text-muted)] hover:bg-[var(--br-surface-muted)] hover:text-[var(--br-chart-secondary)]"}`}>Activity {index + 1}</button>)}</div><LessonActivityPanel key={activity.id} activity={{ id: activity.id, activity_type: activity.activity_type, activity_data: activity.activity_data }} onNext={handleNext} nextLabel={activityIndex < activities.length - 1 ? "Next activity" : "Next slide"} previewOnly /></div>;
}
