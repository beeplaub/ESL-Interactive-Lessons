...
export function MediaRecorderInput({ value, onChange, type = "audio", label, lessonId = "quiz" }: Props) {
  ...
  return (
    <div className="space-y-2 rounded-xl border border-black/10 bg-white p-3 shadow-xs">
      {label && <label className="block text-xs font-semibold text-black/70">{label}</label>}

      {/* Tabs */}
      <div className="flex items-center gap-1 border-b border-black/10 pb-1 text-xs">
        <button
          type="button"
          onClick={() => setActiveTab("url")}
          className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-medium transition ${
            activeTab === "url" ? "bg-moss/10 text-moss font-semibold" : "text-black/55 hover:bg-black/5"
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-medium transition ${
            activeTab === "upload" ? "bg-moss/10 text-moss font-semibold" : "text-black/55 hover:bg-black/5"
          }`}
        >
        ...
        {type === "audio" && (
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-medium transition ${
              activeTab === "record" ? "bg-coral/10 text-coral font-semibold" : "text-black/55 hover:bg-black/5"
            }`}
          >
      ...
      {activeTab === "url" && (
        <div className="flex gap-2">
          <input
            ...
            className="w-full rounded-lg border border-black/15 px-3 py-1.5 text-xs text-ink focus:border-moss focus:outline-hidden"
          />
          {value && (
            <button
              type="button"
              onClick={() => onChange("")}
              className="rounded-lg border border-black/15 px-2 py-1 text-xs text-coral hover:bg-coral/5"
            >
              Clear
            </button>
          )}
      ...
      {activeTab === "upload" && (
        <div className="space-y-1.5">
          <div
            ...
            className="flex cursor-pointer flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed border-black/15 p-3 text-center transition hover:border-moss/50 hover:bg-moss/5"
          >
            <input
              ref={fileInputRef}
              type="file"
        ...
        <div className="space-y-2">
          {recordingState === "idle" && (
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-coral px-3 py-1.5 text-xs font-semibold text-white hover:bg-coral/90"
            >
          ...
          {recordingState === "recording" && (
            <div className="flex items-center justify-between rounded-lg bg-coral/10 p-2 text-xs font-semibold text-coral">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-md bg-coral px-2.5 py-1 text-xs font-semibold text-white hover:bg-coral/90"
              >
                <Square size={12} /> Stop
              </button>
            </div>
          ...
          {recordingState === "recorded" && (
            <div className="space-y-2 rounded-lg bg-black/5 p-2 text-xs">
              <p className="font-semibold text-black/70">Recording finished! Preview:</p>
              {recordedBlobUrl && <audio controls src={recordedBlobUrl} className="w-full h-8" />}
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={saveRecording}
                  className="inline-flex items-center gap-1 rounded-md bg-moss px-3 py-1 text-xs font-semibold text-white hover:bg-moss/90"
                >
                ...
                <button
                  ...
                  className="rounded-md border border-black/15 px-3 py-1 text-xs font-medium text-black/60 hover:bg-black/5"
                >
                  Discard & Re-record
                </button>
              </div>
      ...
      {value && (
        <div className="mt-2 rounded-lg bg-black/5 p-2 space-y-1">
          <p className="text-[10px] uppercase font-bold text-black/40 tracking-wider">Active Media Source:</p>
          {type === "audio" && <audio controls src={value} className="w-full h-8" />}
          {type === "image" && (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={value} alt="Media Preview" className="max-h-24 rounded border border-black/10 object-contain" />
          )}
          {type === "video" && <video controls src={value} className="max-h-36 rounded w-full" />}
          <p className="text-[10px] text-black/40 truncate">{value}</p>
        </div>
      )}
    </div>
  );
