  ...
  return (
    <section className="mt-5 grid gap-5 border-t border-[var(--br-border)] pt-5">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--br-text-muted)]">Write one observable ability per outcome. Map it separately in every course that uses this lesson.</p>
      </div>
      ...
      <div className="grid gap-3">
        {outcomes.map((outcome, index) => (
          <div key={outcome.id} className="rounded-xl border border-[var(--br-border)] bg-[var(--br-surface)] p-3">
            ...
            >
              <input name="code" defaultValue={outcome.code} aria-label="Outcome code" className="rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2 text-sm font-bold" />
              <input name="outcome" defaultValue={outcome.outcome} aria-label="Outcome statement" className="min-w-0 rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2 text-sm" />
              <select name="status" defaultValue={outcome.status} aria-label="Outcome status" className="rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2 text-sm"><option value="ACTIVE">Active</option><option value="ARCHIVED">Archived</option></select>
              <button className="rounded-lg bg-dark px-3 py-2 text-sm font-bold text-on-dark">Save</button>
            </ObeActionForm>
            <div className="mt-2 flex flex-wrap justify-end gap-2">
              <ObeActionForm action={async () => moveLessonOutcome(lessonId, outcome.id, "up")}>
                <button disabled={index === 0} className="grid size-8 place-items-center rounded-lg border border-[var(--br-border)] bg-surface disabled:opacity-30" title="Move outcome up"><ArrowUp size={14} /></button>
              </ObeActionForm>
              <ObeActionForm action={async () => moveLessonOutcome(lessonId, outcome.id, "down")}>
                <button disabled={index === outcomes.length - 1} className="grid size-8 place-items-center rounded-lg border border-[var(--br-border)] bg-surface disabled:opacity-30" title="Move outcome down"><ArrowDown size={14} /></button>
              </ObeActionForm>
              <ObeActionForm action={async () => deleteLessonOutcome(lessonId, outcome.id)} confirmMessage="Delete this lesson outcome and its mappings?">
                <button className="inline-flex h-8 items-center gap-1 rounded-lg border border-coral/25 bg-surface px-2 text-xs font-bold text-coral"><Trash2 size={13} /> Delete</button>
              </ObeActionForm>
            </div>
          </div>
        ))}
        {!outcomes.length ? <p className="rounded-xl border border-dashed border-[var(--br-border)] p-5 text-center text-sm text-[var(--br-text-muted)]">No lesson outcomes yet.</p> : null}
      </div>
      ...
      <ObeActionForm action={addLessonOutcome.bind(null, lessonId)} successMessage="Outcome added." className="grid gap-2 rounded-xl bg-[var(--br-canvas-elevated)] p-3 sm:grid-cols-[90px_1fr_auto]">
        <input name="code" placeholder={`LO${outcomes.length + 1}`} aria-label="New outcome code" className="rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2 text-sm font-bold" />
        <input name="outcome" placeholder="Learners will be able to..." className="min-w-0 rounded-lg border border-[var(--br-border)] bg-surface px-3 py-2 text-sm" />
      ...
      <div className="border-t border-[var(--br-border)] pt-5">
        ...
        <p className="mt-1 text-sm text-[var(--br-text-muted)]">The lesson can be reused. Each placement keeps its own course-outcome mapping.</p>
      </div>

      <ObeActionForm action={placeLessonInCourse.bind(null, lessonId)} successMessage="Lesson added to course." className="grid gap-2 rounded-xl border border-[var(--br-border)] p-3 sm:grid-cols-[1fr_1fr_90px_auto]">
        <select name="courseId" required className="min-w-0 rounded-lg border border-[var(--br-border)] px-3 py-2 text-sm">
          <option value="">Choose course</option>
          {courses.map((course) => <option key={course.id} value={course.id}>{course.title} · {course.status}</option>)}
        </select>
        <select name="sectionId" required className="min-w-0 rounded-lg border border-[var(--br-border)] px-3 py-2 text-sm">
        ...
        <input name="position" type="number" min="1" placeholder="Position" aria-label="Position" className="rounded-lg border border-[var(--br-border)] px-3 py-2 text-sm" />
        <button className="rounded-lg bg-dark px-3 py-2 text-sm font-bold text-on-dark">Add placement</button>
      </ObeActionForm>
      ...
      <div className="grid gap-3">
        {placements.map((placement) => {
          ...
          return (
            <section key={placement.id} className="rounded-xl border border-[var(--br-border)] p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  <p className="font-extrabold">{placement.courses?.title ?? "Course"}</p>
                  <p className="text-xs text-[var(--br-text-muted)]">{placement.course_sections?.title ?? "Section"} · Position {placement.position}</p>
                </div>
