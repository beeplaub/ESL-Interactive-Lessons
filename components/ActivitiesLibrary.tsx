...
export function ActivitiesLibrary({ modules, hasPremiumAccess, isAuthenticated }: { modules: PublishedModule[]; hasPremiumAccess: boolean; isAuthenticated: boolean }) {
  ...
  return <>
    ...
    {modules.filter((item) => item.category === selected).length ? <div className="mt-5 grid gap-3 sm:grid-cols-2">{modules.filter((item) => item.category === selected).map((module) => { const locked = (module.access_type === "PREMIUM" && !hasPremiumAccess) || !isAuthenticated; const href = !isAuthenticated ? `/login?next=${encodeURIComponent(`/activities/${module.id}`)}` : locked ? "/activities/subscribe" : `/activities/${module.id}`; return <Link key={module.id} href={href} className={`rounded-2xl border border-[var(--br-border)] bg-surface p-4 shadow-sm transition hover:border-[var(--br-action)]/50 hover:shadow-md ${locked ? "opacity-90" : ""}`}><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-[var(--br-action)]/10 px-2.5 py-1 text-[11px] font-extrabold text-[var(--br-action)]">{module.level}</span><span className="text-xs font-bold text-[var(--br-text-muted)]">{module.access_type === "PREMIUM" ? "Premium" : "Free"}</span></div><h3 className="mt-4 font-extrabold">{module.title}</h3><p className="mt-1 text-sm leading-5 text-[var(--br-text-muted)]">{module.description || "Focused practice module"}</p><p className="mt-4 text-xs font-extrabold text-[var(--br-action)]">{locked ? isAuthenticated ? "See access options →" : "Sign in to practise →" : "Open module →"}</p></Link>; })}</div> : <div className="mt-5 rounded-2xl border border-dashed border-[var(--br-border)] bg-[var(--br-surface-muted)]/55 px-5 py-12 text-center"><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-[var(--br-action)]/10 text-[var(--br-action)]"><Sparkles size={21} /></div><h3 className="mt-4 text-lg font-extrabold">No {category.label.toLowerCase()} activities yet</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--br-text-muted)]">New {category.label.toLowerCase()} practice will appear here when it is published.</p></div>}
  </>;
}
