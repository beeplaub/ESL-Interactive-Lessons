...
function TableCompletionPlayer({ question, value, disabled, onChange }: { question: QuizQuestion; value: Record<string, string>; disabled: boolean; onChange: (value: Record<string, string>) => void }) {
  ...
  return <div className="grid gap-4">
    ...
    {sourceUrl && sourceType === "AUDIO" ? <div className="rounded-2xl border border-[var(--br-surface-strong)] bg-[var(--br-canvas-elevated)] p-4"><p className="mb-2 text-xs font-bold uppercase tracking-wide text-[var(--br-text-muted)]">Listen for the information</p><AudioTrackPlayer value={sourceUrl} /></div> : null}
    ...
    <div className="hidden overflow-x-auto rounded-2xl border border-[var(--br-surface-strong)] sm:block"><table className="w-full min-w-[620px] border-collapse text-left text-sm"><thead><tr className="bg-moss text-on-dark"><th className="px-4 py-3 font-extrabold">Details</th>{columns.map((column) => <th key={String(column.id)} className="px-4 py-3 font-extrabold">{String(column.label ?? "")}</th>)}</tr></thead><tbody>{rows.map((row, rowIndex) => { const rowId = String(row.id ?? `row-${rowIndex + 1}`); const cells = asRecord(row.cells as Json); return <tr key={rowId} className="border-t border-[var(--br-surface-strong)] align-top odd:bg-[var(--br-canvas-elevated)]"><th className="w-36 px-4 py-3 font-bold text-[var(--br-dark-card)]">{String(row.label ?? "")}</th>{columns.map((column) => { const columnId = String(column.id); const cell = asRecord(cells[columnId] as Json); const key = `${rowId}:${columnId}`; const blank = cell.blank === true; if (!blank) return <td key={columnId} className="px-4 py-3 font-semibold text-[var(--br-dark-card)]">{String(cell.value ?? "")}</td>; const isSelect = String(cell.mode ?? "WRITE").toUpperCase() === "SELECT" || String(cell.input_type ?? "").toUpperCase() === "DROPDOWN"; const cellOptions = Array.isArray(cell.options) ? cell.options.map(String).filter(Boolean) : []; return <td key={columnId} className="bg-[var(--br-chart-primary)]/8 px-3 py-3">{isSelect ? <select aria-label={`${row.label ?? "Row"} ${column.label ?? "cell"}`} disabled={disabled} value={value[key] ?? ""} onChange={(event) => onChange({ ...value, [key]: event.target.value })} className="w-full rounded-xl border-2 border-[var(--br-chart-primary)]/45 bg-white px-3 py-2 font-semibold text-[var(--br-dark-card)] shadow-sm focus:border-[var(--br-chart-primary)] focus:outline-none"><option value="">Choose an answer…</option>{cellOptions.map((option) => <option key={option} value={option}>{option}</option>)}{cellOptions.length === 0 ? <option value={String(cell.answer ?? "")}>{String(cell.answer ?? "Add options in the editor")}</option> : null}</select> : <input aria-label={`${row.label ?? "Row"} ${column.label ?? "cell"}`} disabled={disabled} value={value[key] ?? ""} onChange={(event) => onChange({ ...value, [key]: event.target.value })} placeholder="Type answer" className="w-full rounded-xl border-2 border-[var(--br-chart-primary)]/45 bg-white px-3 py-2 font-semibold text-[var(--br-dark-card)] placeholder:text-[var(--br-text-muted)] shadow-sm focus:border-[var(--br-chart-primary)] focus:outline-none" />}</td>; })}</tr>; })}</tbody></table></div>
    <div className="grid gap-3 sm:hidden">{rows.map((row, rowIndex) => { const rowId = String(row.id ?? `row-${rowIndex + 1}`); const cells = asRecord(row.cells as Json); return <section key={rowId} className="rounded-2xl border border-[var(--br-surface-strong)] bg-[var(--br-canvas-elevated)] p-3 shadow-sm"><h4 className="font-extrabold text-[var(--br-dark-card)]">{String(row.label ?? "")}</h4><div className="mt-2 grid gap-2">{columns.map((column) => { const columnId = String(column.id); const cell = asRecord(cells[columnId] as Json); const key = `${rowId}:${columnId}`; if (cell.blank !== true) return <div key={columnId} className="flex items-start justify-between gap-3 rounded-xl bg-white px-3 py-2 text-sm"><span className="text-xs font-bold text-[var(--br-text-muted)]">{String(column.label ?? "")}</span><span className="text-right font-semibold text-[var(--br-dark-card)]">{String(cell.value ?? "")}</span></div>; const isSelect = String(cell.mode ?? "WRITE").toUpperCase() === "SELECT" || String(cell.input_type ?? "").toUpperCase() === "DROPDOWN"; const cellOptions = Array.isArray(cell.options) ? cell.options.map(String).filter(Boolean) : []; return <label key={columnId} className="rounded-xl border-2 border-[var(--br-chart-primary)]/35 bg-[var(--br-chart-primary)]/10 p-3 text-xs font-bold text-[var(--br-dark-card)]">{String(column.label ?? "")}{isSelect ? <select disabled={disabled} value={value[key] ?? ""} onChange={(event) => onChange({ ...value, [key]: event.target.value })} className="mt-1 w-full rounded-xl border border-[var(--br-chart-primary)]/45 bg-white px-3 py-2 text-sm font-semibold"><option value="">Choose an answer…</option>{cellOptions.map((option) => <option key={option} value={option}>{option}</option>)}{cellOptions.length === 0 ? <option value={String(cell.answer ?? "")}>{String(cell.answer ?? "Add options in the editor")}</option> : null}</select> : <input disabled={disabled} value={value[key] ?? ""} onChange={(event) => onChange({ ...value, [key]: event.target.value })} placeholder="Type answer" className="mt-1 w-full rounded-xl border border-[var(--br-chart-primary)]/45 bg-white px-3 py-2 text-sm font-semibold" />}</label>; })}</div></section>; })}</div>
...
function Pronunciation({
  ...
  return (
    <div className="relative grid gap-4 overflow-hidden rounded-[24px] border border-violet-200/80 bg-gradient-to-br from-violet-50 via-white to-orange-50 p-5 text-center shadow-sm sm:p-6">
      ...
      {level === "word" ? <div className="relative z-10 grid gap-2 text-left">{targets.map((target) => {
        const isActive = activeKey === target.id && micState === "listening";
        const anotherIsActive = micState === "listening" && !isActive;
        const isUnavailable = (attemptsUsed[target.id] ?? 0) >= maxAttempts || results[target.id] === true;
        return <div key={target.id} className={`flex flex-wrap items-center justify-between gap-3 rounded-[14px] border p-3 transition ${isActive ? "border-[var(--br-action)] bg-[var(--br-action)]/10 shadow-md ring-2 ring-[var(--br-action)]/20" : "border-white/80 bg-white/75"}`}>
          <div><p className="font-bold" style={{ color: target.color }}>{target.text}</p><p className="text-xs text-[var(--br-text-muted)]">{isActive ? `Listening · ${Math.max(0, 12 - seconds)}s remaining` : renderStatus(target)}{attemptsUsed[target.id] ? ` · ${attemptsUsed[target.id]} attempt${attemptsUsed[target.id] === 1 ? "" : "s"}` : ""}</p></div>
          <button type="button" disabled={disabled || micState === "transcribing" || anotherIsActive || isUnavailable} onClick={() => (isActive ? void finishRecording() : void recordFor(target.id, [target]))} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-black text-white shadow-sm transition disabled:cursor-not-allowed disabled:opacity-50 ${isActive ? "bg-coral ring-2 ring-coral/30 animate-pulse" : results[target.id] ? "bg-emerald-600" : "bg-[var(--br-action)] hover:bg-[var(--br-action-strong)]"}`} aria-label={isActive ? `Stop recording ${target.text}` : `Record ${target.text}`}>
