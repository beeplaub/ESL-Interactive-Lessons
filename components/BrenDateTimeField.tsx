...
export function BrenDateTimeField({ name, defaultValue = "", label }: Props) {
  ...
  return <div className="relative"><input type="hidden" name={name} value={value} /><button type="button" onClick={(event) => { event.stopPropagation(); setOpen(true); }} className="field flex h-10 w-full items-center justify-between gap-2 text-left text-sm"><span className={value ? "text-ink" : "text-[var(--br-text-muted)]"}>{display}</span><CalendarDays size={16} className="shrink-0 text-[var(--br-brand)]" /></button>{open ? <div className="fixed inset-0 z-[110] grid place-items-center bg-black/35 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" onClickCapture={(event) => event.stopPropagation()} onMouseDownCapture={(event) => event.stopPropagation()} onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}><div className="w-full max-w-xs rounded-2xl border border-[var(--br-border)] bg-surface p-3 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wider text-[var(--br-brand)]">Schedule</p><h3 className="mt-0.5 text-sm font-semibold">Choose date and time</h3></div><button type="button" onClick={() => setOpen(false)} className="grid size-8 place-items-center rounded-full border border-[var(--br-border)]" aria-label="Close date picker"><X size={15} /></button></div><div className="mt-2 flex items-center justify-between"><button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} className="grid size-8 place-items-center rounded-lg hover:bg-[var(--br-surface-muted)]"><ChevronLeft size={16} /></button><p className="text-sm font-bold">{month.toLocaleDateString(undefined, { month: "long", year: "numeric" })}</p><button type="button" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} className="grid size-8 place-items-center rounded-lg hover:bg-[var(--br-surface-muted)]"><ChevronRight size={16} /></button></div><div className="mt-1 grid grid-cols-7 gap-0.5 text-center text-[10px] font-bold text-[var(--br-text-muted)]">{["S", "M", "T", "W", "T", "F", "S"].map((day, i) => <span key={`${day}-${i}`} className="py-1">{day}</span>)}{days.map((day, i) => day ? <button type="button" key={day} onClick={() => setDate(`${month.getFullYear()}-${String(month.getMonth() + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`)} className={`grid aspect-square place-items-center rounded-md text-xs ${date === `${month.getFullYear()}-${String(month.getMonth() + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}` ? "bg-[var(--br-brand)] text-on-dark" : "hover:bg-[var(--br-brand)]/10"}`}>{day}</button> : <span key={`empty-${i}`} />)}</div><label className="mt-2 block text-xs font-semibold"><span className="flex items-center gap-1"><Clock3 size={13} /> Time</span><input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="field mt-1 h-9 w-full text-sm" /></label><div className="mt-3 flex justify-end gap-1.5"><button type="button" onClick={() => { setValue(""); setDate(""); setOpen(false); }} className="rounded-lg border border-[var(--br-border)] px-2.5 py-1.5 text-xs font-bold">Clear</button><button type="button" onClick={chooseToday} className="rounded-lg border border-[var(--br-border)] px-2.5 py-1.5 text-xs font-bold">Today</button><button type="button" onClick={apply} className="rounded-lg bg-[var(--br-brand)] px-3 py-1.5 text-xs font-bold text-on-dark">Apply</button></div></div></div> : null}</div>;
}
